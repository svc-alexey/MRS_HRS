...
Процедура УдалитьМеню(Команда)
	...
	Если ЗначениеЗаполнено(текСтрока) Тогда
		...
		// Для тестирования: сразу обработать очередь удаления
		ОбработатьОчередьУдаленияСервер(текСтрока);
...
// Обрабатывает очередь удаления позиции в Simphony (для тестирования)
//
// Параметры:
//  МенюДоУдаления - СправочникСсылка.питМеню - ссылка на позицию (уже может быть удалена)
//
&НаСервере
Процедура ОбработатьОчередьУдаленияСервер(Знач МенюДоУдаления)
	
	// Получаем записи из очереди с операцией DELETE и всеми необходимыми данными
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	Очередь.ПозицияМеню = &ПозицияМеню
		|	И Очередь.Операция = ""DELETE""";
	
	Запрос.УстановитьПараметр("ПозицияМеню", МенюДоУдаления);
	...
	Пока Выборка.Следующий() Цикл
		...
		// Сразу отправляем DELETE запрос в Simphony
		// Функция УдалитьПозициюИзSimphony сама получит stringNumberId из очереди (поле ТекстОшибки)
		РезультатУдаления = MRS_ИнтеграцияSimphonyСервер.УдалитьПозициюИзSimphony(
			Выборка.ПозицияМеню,
		...
		Если РезультатУдаления.Успешно Тогда
			...
			ТекстСообщения = СтрШаблон("Позиция успешно удалена из Simphony. %1", РезультатУдаления.Сообщение);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			
		Иначе
			
			ТекстСообщения = СтрШаблон("Ошибка удаления из Simphony (HTTP %1): %2", 
