...
Процедура ПриОткрытии(Отказ)
	...
	// Инициализируем реквизиты формы для работы с деревом
	ФлагИзмененияОтбораДерева = Ложь;
...
Процедура ДеревоПриАктивизацииСтроки(Элемент)
	...
	// Проверяем флаг, чтобы избежать циклического вызова
	Если ФлагИзмененияОтбораДерева Тогда
		Возврат;
	КонецЕсли;
	
	ТекущиеДанные = Элемент.ТекущаяСтрока;
	...
	Если ТекущийРодительДерева = ТекущиеДанные Тогда
		Возврат;
	КонецЕсли;
	
	// Сохраняем текущего родителя
	ТекущийРодительДерева = ТекущиеДанные;
	
	// Устанавливаем флаг перед изменением отбора
	ФлагИзмененияОтбораДерева = Истина;
	
	// Устанавливаем отбор списка по выбранной группе
	ИспользоватьОтбор = ЗначениеЗаполнено(ТекущиеДанные);
	ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(Список, "Родитель", ТекущиеДанные, ИспользоватьОтбор);
	
	// Сохраняем текущего родителя для создания новых элементов
	Элементы.Список.ТекущийРодитель = ТекущиеДанные;
	
	// Сбрасываем флаг через отложенный вызов
	ПодключитьОбработчикОжидания("СброситьФлагИзмененияОтбора", 0.1);
	
КонецПроцедуры

&НаКлиенте
Процедура СброситьФлагИзмененияОтбора()
