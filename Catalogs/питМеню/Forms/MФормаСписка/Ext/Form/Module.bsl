...
// Обрабатывает стандартное оповещение о записи справочника питМеню.
// Автоматически обновляет динамический список при изменении позиций.
//
// Параметры:
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник) Экспорт
	...
	Если ИмяСобытия = "Запись_питМеню" Тогда
...
// Обработчик перед началом изменения элемента в списке.
// Перехватывает стандартное открытие формы элемента и передаёт обработчик закрытия.
//
// Параметры:
//  Элемент - ПолеФормы - элемент формы "Список"
...
Процедура СписокПередНачаломИзменения(Элемент, Отказ)
	...
	// Открываем форму элемента с обработчиком закрытия
...
Процедура ПослеЗакрытияФормы(Результат, ДопПараметры) Экспорт
	...
	// Обновляем список, если запись прошла (при создании или редактировании)
	// Не обновляем только при явном отказе (Результат = Ложь)
