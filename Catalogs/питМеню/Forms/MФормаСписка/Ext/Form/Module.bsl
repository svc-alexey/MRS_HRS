...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Настройка условного оформления для отображения статуса "Выключен"
...
// Настраивает условное оформление для отображения статуса "Выключен" в списке позиций меню.
// Добавляет отдельную колонку с картинкой для визуального отображения доступности позиции.
//
&НаСервере
Процедура НастроитьУсловноеОформлениеВыключен()
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Список.Выключен");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
	
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Картинка", БиблиотекаКартинок.Отклонить);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ОтображатьТекст", Ложь);
	...
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Картинка", БиблиотекаКартинок.Подтвердить);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ОтображатьТекст", Ложь);
...
//   * АдресДокумента - Строка - адрес незаписанного документа во временном хранилище
...
Функция СоздатьДокументУстановкиЦенНаСервере(Знач Режим, Знач ВыделенныеСтроки)
	...
	Попытка
		...
		// Сбор номенклатуры из позиций меню с расчетом цены по наценке
		// Регистр СтоимостьТоваров находится в основной конфигурации
		
		//BSLLS:QueryUnknownMetadata-off
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтоимостьТоваров.СрезПоследних(&ТекущаяДата, ) КАК СтоимостьТоваровСрезПоследних //BSLLS:QueryUnknownMetadata-off //BSLLS:VirtualTableWithoutParameters-off
		...
		Запрос.УстановитьПараметр("ТекущаяДата", Дата(1, 1, 1));
		...
		// Заполнение табличных частей Товары и ЦеныНоменклатуры
		КоличествоПозиций = 0;
		
		Для Каждого СтрокаДанных Из ТаблицаДанных Цикл
			
			// Добавление в табличную часть Товары
			НоваяСтрокаТовары = НовыйДокумент.Товары.Добавить();
			НоваяСтрокаТовары.Номенклатура = СтрокаДанных.Номенклатура;
			
			// Заполнение цены, если вид цен определен и цена рассчитана
			Если ЗначениеЗаполнено(ВидЦен) И ЗначениеЗаполнено(СтрокаДанных.СтоимостьСНаценкой) Тогда
				
				НоваяСтрокаЦены = НовыйДокумент.ЦеныНоменклатуры.Добавить();
				НоваяСтрокаЦены.Номенклатура = СтрокаДанных.Номенклатура;
				НоваяСтрокаЦены.ВидЦены = ВидЦен;
				НоваяСтрокаЦены.Цена = СтрокаДанных.СтоимостьСНаценкой;
