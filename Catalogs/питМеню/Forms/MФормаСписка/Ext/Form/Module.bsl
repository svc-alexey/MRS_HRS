...
Процедура УстановитьОтборПоВыбраннойГруппе()
	...
	Если ИспользоватьОтбор Тогда
		...
		// Определяем, является ли выбранная группа группой верхнего уровня
		ЭтоГруппаВерхнегоУровня = ПроверитьЯвляетсяГруппойВерхнегоУровня(ТекущийРодительДерева);
		
		Если ЭтоГруппаВерхнегоУровня Тогда
			
			// Для группы верхнего уровня используем рекурсивный отбор "В ИЕРАРХИИ"
			УстановитьОтборРодительВИерархии(ТекущийРодительДерева);
			
		Иначе
			
			// Для группы не верхнего уровня используем обычный отбор по равенству
			ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(Список, "Родитель", ТекущийРодительДерева, Истина);
			
		КонецЕсли;
		
	Иначе
		
		// Снимаем отбор по родителю
		ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(Список, "Родитель", ТекущийРодительДерева, Ложь);
	...
	ИначеЕсли ВидСписка = "ИсключитьСтопЛист" Тогда
		...
		ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(Дерево, "Выключен", Ложь, Ложь);
	Иначе
		// Неизвестный вид списка - не устанавливаем отбор
		ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(Список, "Выключен", Ложь, Ложь);
		ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(Дерево, "Выключен", Ложь, Ложь);
	КонецЕсли;  
...
// Проверяет, является ли указанная группа группой верхнего уровня.
//
// Параметры:
//  Группа - СправочникСсылка.питМеню - проверяемая группа
//
// Возвращаемое значение:
//  Булево - Истина, если группа верхнего уровня (без родителя)
//
&НаСервере
Функция ПроверитьЯвляетсяГруппойВерхнегоУровня(Знач Группа)
	
	Если Не ЗначениеЗаполнено(Группа) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат Не ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Группа, "Родитель"));
	
КонецФункции

// Устанавливает отбор списка по родителю с использованием рекурсивной выборки подгрупп.
//
// Параметры:
//  ЗначениеРодителя - СправочникСсылка.питМеню - группа, по иерархии которой устанавливается отбор
//
&НаСервере
Процедура УстановитьОтборРодительВИерархии(Знач ЗначениеРодителя)
	
	// Получаем список всех подгрупп выбранной группы (включая саму группу)
	МассивГрупп = ПолучитьВсеПодгруппыГруппы(ЗначениеРодителя);
	
	// Находим элемент отбора по полю "Родитель"
	...
	Для Каждого ТекЭлементОтбора Из Список.Отбор.Элементы Цикл
		...
		Если ТипЗнч(ТекЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных") 
			И Строка(ТекЭлементОтбора.ЛевоеЗначение) = "Родитель" Тогда
	...
	// Если элемент отбора не найден, создаем его
	...
	// Устанавливаем отбор по списку групп
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
	ЭлементОтбора.ПравоеЗначение = МассивГрупп;
	ЭлементОтбора.Использование = Истина;
	
КонецПроцедуры

// Получает все подгруппы указанной группы рекурсивно.
//
// Параметры:
//  Группа - СправочникСсылка.питМеню - группа, для которой получаются подгруппы
//
// Возвращаемое значение:
//  Массив из СправочникСсылка.питМеню - массив групп (включая переданную группу)
//
&НаСервере
Функция ПолучитьВсеПодгруппыГруппы(Знач Группа)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	питМеню.Ссылка В ИЕРАРХИИ(&Группа)
		|	И питМеню.ЭтоГруппа";
