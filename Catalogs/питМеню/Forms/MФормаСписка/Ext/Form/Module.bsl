...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(Дерево, "Владелец", ОтборВладелец, Истина);
	...
	ВидСписка = "ВесьСписок";
	
	ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(Список, "Выключен", Ложь, Ложь);
	//ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(Дерево, "Выключен", Ложь, Ложь);
...
// Обработчик оповещения об изменениях.
// Автоматически вызывается при генерации оповещений из других форм.
//
// Параметры:
//  ИмяСобытия - Строка - имя события
//  Параметр - Произвольный - параметры события
//  Источник - Произвольный - источник события
//
&НаКлиенте
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник) Экспорт
	
	Если ИмяСобытия = "Запись_питМеню" Или ИмяСобытия = "Записан_Объект" Тогда
...
Процедура УдалитьМеню(Команда)
	...
	ТекстВопроса = "Вы уверены, что хотите удалить позицию меню в 1С и Simphony?" + Символы.ПС +
	...
	ПоказатьВопрос(
		Новый ОписаниеОповещения("УдалитьЗавершение", ЭтотОбъект, ДополнительныеПараметры),
		ТекстВопроса,
...
&НаКлиенте
// Обработчик завершения диалога подтверждения Удаления позиции меню.
//
// Параметры:
//  РезультатВопроса - КодВозвратаДиалога - результат диалога.
...
Процедура УдалитьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ДополнительныеПараметры.текСтрока) Тогда
		УдалитьМенюСервер(ДополнительныеПараметры.текСтрока);
...
Процедура УстановитьОтборПоВыбраннойГруппе()
	...
	Если ВидСписка = "ВесьСписок" Тогда
		ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(Список, "Выключен", Ложь, Ложь);
		//ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(Дерево, "Выключен", Ложь, Ложь);
	ИначеЕсли ВидСписка = "СтопЛист" Тогда
		ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(Список, "Выключен", Истина, Истина);
		//ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(Дерево, "Выключен", Ложь, Ложь);
	ИначеЕсли ВидСписка = "ИсключитьСтопЛист" Тогда
		ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(Список, "Выключен", Ложь, Истина);
		//ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(Дерево, "Выключен", Ложь, Ложь);
	КонецЕсли;  
...
Функция СоздатьДокументУстановкиЦенНаСервере(Знач Режим, Знач ВыделенныеСтроки)
	...
	Попытка
		...
		ИначеЕсли Режим = "Группа" Тогда
			...
			ТекущийРодитель = ВыделенныеСтроки[0].Родитель;
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	питМеню.Рецептура КАК Рецептура
		               |ИЗ
		               |	Справочник.питМеню КАК питМеню
		...
		НовыйДокумент.Статус = Перечисления.СтатусыУстановокЦенНоменклатуры.НеСогласован;
		...
		Для Каждого СтрокаДанных Из ТаблицаДанных Цикл
			...
			// Если нет расчета, возможно рецептура новая, пробуем расчитать по логике общепита
			Если СтрокаДанных.СтоимостьСНаценкой = 0 И ЗначениеЗаполнено(СтрокаДанных.Рецептура) Тогда
				СтоимостьРецептуры = MRS_ИнтеграцияSimphonyСервер.ПолучитьСтоимостьПоРецептуре(СтрокаДанных);
				НоваяСтрокаТовары.Цена = СтоимостьРецептуры; 
			Иначе	
