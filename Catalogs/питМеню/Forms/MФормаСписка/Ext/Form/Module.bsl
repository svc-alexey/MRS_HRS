...
Функция СоздатьДокументУстановкиЦенНаСервере(Знач Режим, Знач ВыделенныеСтроки)
	...
	Попытка
		...
		Параметр.Добавить(Справочники.питМеню.ПолучитьСсылку(Новый УникальныйИдентификатор("ADC77C1C-CB8F-11F0-A7E7-FF86E051A10B")));
		Запрос.УстановитьПараметр("МассивПозиций", Параметр);
		Запрос.УстановитьПараметр("ВидМеню", Справочники.питВидыМеню.ПолучитьСсылку(Новый УникальныйИдентификатор("B255AE45-BBE9-11F0-A7E7-FF86E051A10B")));// Меню Азур (Справочники.питВидыМеню)
