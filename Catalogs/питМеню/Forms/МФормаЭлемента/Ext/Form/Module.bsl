...
Процедура УстановитьЗначениеРеквизитовПриСменеНоменклатуры()
	...
	Если ЗначениеЗаполнено(Объект.Номенклатура) Тогда
		...
		Если Объект.Номенклатура.питВидНоменклатуры = Перечисления.питВидыНоменклатуры.Блюдо Тогда
			Объект.Рецептура = питПроизводство.ВернутьОсновнуюРецептуру(
				Объект.Номенклатура,
				,
				Перечисления.питХозяйственныеОперации.РецептураПриготовление,
				ТекущаяДатаСеанса(),
				СпособРасчетаРецептур);
		Иначе
...
Процедура ВыключенПриИзмененииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		// Оповещаем форму списка об изменении
		Оповестить("ОбновитьСписокМенюSimphony", Объект.Ссылка);
