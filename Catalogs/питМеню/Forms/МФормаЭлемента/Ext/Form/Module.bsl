...
Функция СгенерироватьКодSimphony()
	...
	ПерваяЦифра = Число(Лев(СтрокаКодMajorGroup, 1));
	...
	МинГраница = ПерваяЦифра * 1000000000; // Умножаем на 10^9
	МаксГраница = (ПерваяЦифра + 1) * 1000000000;
...
Процедура УстановитьЗначениеРеквизитовПриСменеНоменклатуры()
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) И НЕ ЗначениеЗаполнено(Объект.MRS_КодSimphony) Тогда
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) И ЗначениеЗаполнено(Объект.Номенклатура) И НЕ ЗначениеЗаполнено(Объект.MRS_КодSimphony) Тогда
		Объект.MRS_КодSimphony = СгенерироватьКодSimphony();
	КонецЕсли;
