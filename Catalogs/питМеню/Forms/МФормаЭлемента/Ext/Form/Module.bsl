...
//
// Параметры:
//  Номенклатура - СправочникСсылка.Номенклатура - проверяемый элемент номенклатуры.
//  ТекстОшибки - Строка - текст ошибки, если вид номенклатуры не допустимый.
//
...
&НаСервере
// Устанавливает значения реквизитов формы при смене номенклатуры.
// Заполняет наименование, рецептуру, единицу измерения и генерирует код Simphony.
//
Процедура УстановитьЗначениеРеквизитовПриСменеНоменклатуры()
	...
	Если ЗначениеЗаполнено(Объект.Владелец) Тогда
		СтруктураРеквизитыВидаМеню = MRS_ИнтеграцияSimphonyСервер.ПолучитьРеквизитыВидаМеню(Объект.Владелец);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Объект.Номенклатура) Тогда
		...
		Если Объект.Номенклатура.питВидНоменклатуры = Перечисления.питВидыНоменклатуры.Блюдо Тогда	
			Объект.Рецептура = питПроизводство.ВернутьОсновнуюРецептуру(Объект.Номенклатура,, Перечисления.питХозяйственныеОперации.РецептураПриготовление,
				ТекущаяДатаСеанса(), СпособРасчетаРецептур, , СтруктураРеквизитыВидаМеню.Организация,,,СтруктураРеквизитыВидаМеню.Подразделение);
		Иначе
	...
	Если ЗначениеЗаполнено(СтруктураРеквизитыВидаМеню.СтавкаНДС) Тогда
		Объект.MRS_СтавкаНДС = СтруктураРеквизитыВидаМеню.СтавкаНДС;
	Иначе
		УчетНДСУПВызовСервера.СтавкаНДСПоУмолчанию(СтруктураРеквизитыВидаМеню.Организация,ТекущаяДатаСеанса());
	КонецЕсли;	
...
&НаСервере
// Проверяет уникальность товара в прайсе.
//
// Возвращаемое значение:
//  Булево - Истина, если товар уникален в прайсе или проверка не требуется.
//
...
&НаСервере
// Записывает объект после успешной синхронизации видимости с флагом пропуска очереди.
//
Процедура ЗаписатьОбъектПослеСинхронизацииВидимости()

	ОбъектДанных = РеквизитФормыВЗначение("Объект");
	ОбъектДанных.ДополнительныеСвойства.Вставить("ПропуститьДобавлениеВОчередьSimphony", Истина);
...
&НаКлиенте
// Обработчик изменения поля "Номенклатура".
// Проверяет допустимость вида номенклатуры и устанавливает значения реквизитов формы.
//
// Параметры:
//  Элемент - ЭлементФормы - элемент формы поля "Номенклатура".
//
...
//
// Параметры:
//  Элемент - ЭлементФормы - элемент формы флага "Выключен".
//
...
Процедура ВыключенПриИзмененииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		// Записываем объект с флагом, чтобы не добавлять повторно в очередь
		ЗаписатьОбъектПослеСинхронизацииВидимости();
...
// Открывает форму списка регистра сведений с отбором по текущей позиции меню.
//
// Параметры:
//  Элемент - ЭлементФормы - элемент формы гиперссылки.
//
...
// Инициализирует форму, подключает стандартные подсистемы и управление свойствами.
//
// Параметры:
...
// Загружает дополнительные реквизиты и генерирует код Simphony для нового элемента.
//
// Параметры:
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		// Для существующего элемента ничего не делаем
	Иначе
...
// Проверяет и заполняет обязательные реквизиты перед записью.
//
// Параметры:
//  Отказ - Булево - признак отказа от записи.
//  ТекущийОбъект - ДокументОбъект - объект документа для записи.
...
&НаКлиенте
// Обработчик события перед записью на клиенте.
// Проверяет уникальность товара в прайсе перед записью.
//
// Параметры:
//  Отказ - Булево - признак отказа от записи.
//  ПараметрыЗаписи - Структура - параметры записи.
...
&НаСервере
// Обработчик события чтения данных на сервере.
// Обрабатывает чтение данных для управления свойствами.
//
// Параметры:
//  ТекущийОбъект - ДокументОбъект - объект документа для чтения.
//
...
// Обрабатывает оповещения для управления свойствами.
//
// Параметры:
...
&НаКлиенте
// Подключаемый обработчик изменения дополнительного реквизита.
//
// Параметры:
//  Элемент - ЭлементФормы - элемент формы дополнительного реквизита.
//
...
&НаСервере
// Обновляет элементы дополнительных реквизитов на форме.
//
...
&НаКлиенте
// Подключаемый обработчик выполнения команды свойств.
//
// Параметры:
//  ЭлементИлиКоманда - Произвольный - элемент формы или команда.
//  НавигационнаяСсылка - Произвольный - навигационная ссылка.
//  СтандартнаяОбработка - Булево - признак стандартной обработки.
//
...
&НаСервере
// Обработчик проверки заполнения на сервере.
//
// Параметры:
//  Отказ - Булево - признак отказа от записи.
