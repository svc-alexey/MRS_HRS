...
Процедура ПередУдалением(Отказ)
	...
	Если ЗначениеЗаполнено(MRS_КодSimphony) Тогда
		...
		Если ЗначениеЗаполнено(КассовыйУзел) Тогда
			// Сохраняем stringNumberId для операции DELETE
			MRS_ИнтеграцияSimphonyСервер.ДобавитьВОчередьВыгрузкиСПараметрами(Ссылка, "DELETE", КассовыйУзел, MRS_КодSimphony);
		КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	// Получаем кассовый узел для проверки
	КассовыйУзел = ПолучитьКассовыйУзелДляПозиции(Владелец);
	
	// Проверяем наличие кассового узла
	Если НЕ ЗначениеЗаполнено(КассовыйУзел) Тогда
		
		ТекстОшибки = СтрШаблон("Кассовый узел не найден. Добавьте вид меню ""%1"" в кассовый узел.", Владелец);
		
		// Выдаем ошибку и отменяем запись
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, ЭтотОбъект, , , Отказ);
		Возврат;
		
	КонецЕсли;
	
	// Проверяем заполненность кода Simphony
	Если НЕ ЗначениеЗаполнено(MRS_КодSimphony) Тогда
		
		// Проверяем наличие необходимых реквизитов для генерации кода
		ТекстОшибки = "";
		ПричиныОтсутствия = Новый Массив;
		...
		Если ЗначениеЗаполнено(Номенклатура) Тогда
			...
			Запрос.Текст = 
				...
				|	Номенклатура.ГруппаАналитическогоУчета.MRS_КодFamilyGroup КАК КодFamilyGroup
				|ИЗ
			...
			Если Выборка.Следующий() Тогда
				...
				// Проверяем наличие кода MajorGroup (M Group)
				Если НЕ ЗначениеЗаполнено(Выборка.КодMajorGroup) Тогда
					ПричиныОтсутствия.Добавить("не заполнен номер M Group (Major Group) в классификаторе ГАУ номенклатуры");
				КонецЕсли;
				
				// Проверяем наличие кода FamilyGroup (F Group)
				Если НЕ ЗначениеЗаполнено(Выборка.КодFamilyGroup) Тогда
					ПричиныОтсутствия.Добавить("не заполнен номер F Group (Family Group) в классификаторе ГАУ номенклатуры");
				КонецЕсли;
				
			КонецЕсли;
			
		Иначе
			ПричиныОтсутствия.Добавить("не заполнена номенклатура");
		КонецЕсли;
		
		// Если есть причины отсутствия кода Simphony
		Если ПричиныОтсутствия.Количество() > 0 Тогда
			
			ТекстОшибки = "Код Simphony не может быть сгенерирован: " + СтрСоединить(ПричиныОтсутствия, "; ");
			
			// Записываем статус в регистр очереди, если есть кассовый узел
			Если ЗначениеЗаполнено(КассовыйУзел) Тогда
				
				НаборЗаписей = РегистрыСведений.MRS_ОчередьВыгрузкиМенюSimphony.СоздатьНаборЗаписей();
				НаборЗаписей.Отбор.ПозицияМеню.Установить(Ссылка);
				НаборЗаписей.Отбор.КассовыйУзел.Установить(КассовыйУзел);
				
				Запись = НаборЗаписей.Добавить();
				Запись.ПозицияМеню = Ссылка;
				Запись.КассовыйУзел = КассовыйУзел;
				Запись.Операция = "";
				Запись.Статус = Перечисления.MRS_СтатусыСинхронизацииSimphony.НеВыгружено;
				Запись.ДатаПостановкиВОчередь = ТекущаяДатаСеанса();
				Запись.КоличествоПопыток = 0;
				Запись.СледующаяПопытка = Дата(1, 1, 1);
				Запись.ТекстОшибки = Лев(ТекстОшибки, 500);
	...
	// Проверяем наличие записи в регистре очереди
	НаборЗаписей = РегистрыСведений.MRS_ОчередьВыгрузкиМенюSimphony.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.ПозицияМеню.Установить(Ссылка);
	НаборЗаписей.Отбор.КассовыйУзел.Установить(КассовыйУзел);
	...
	Если НаборЗаписей.Количество() = 0 Тогда
		...
		// Записи нет в очереди - требуется выгрузка
	...
	Если ТребуетсяВыгрузка И ЗначениеЗаполнено(КассовыйУзел) Тогда
		
		// Проверяем флаг пропуска добавления в очередь (установлен после синхронизации видимости)
		Если ДополнительныеСвойства.Свойство("ПропуститьДобавлениеВОчередьSimphony")
			И ДополнительныеСвойства.ПропуститьДобавлениеВОчередьSimphony Тогда
			
			// Синхронизация уже выполнена, пропускаем добавление в очередь
			Возврат;
			
		КонецЕсли;
		
		// Определяем операцию на основе истории синхронизации
		Операция = "CREATE";
		...
		Запрос.УстановитьПараметр("ПозицияМеню", Ссылка);
