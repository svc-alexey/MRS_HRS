...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	//<<Портал-Юг, Давиденко 2019.02.14 [3.2.1 Автоматическое заполнение организации по складу, указанному в документе]
	ПЛ_Сервер.УстановитьОрганизациюИзСкладаВДокументахПеремещения(СкладОтправитель, СкладПолучатель, Организация);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// << MRS #ERP-5469 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 21.10.25, ()
	Если Ссылка.ПометкаУдаления И Ссылка.ПЛ_ДокументВыгрузкиВБП Тогда //И Ссылка.Комментарий = "#Помеченный на удаление обработкой создания межфирменных" Тогда
		ОбщегоНазначения.СообщитьПользователю("Запрещено изменение документа, помеченного на удаление обработкой создания межфирменных!",,,,Отказ);
		Возврат;
	КонецЕсли;
	// >> MRS #ERP-5469 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 21.10.25, ()
	
	// << MRS #HELP1C-12035 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 04.03.25, ()
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение И Дата > ТекущаяДатаСеанса() Тогда
		ОбщегоНазначения.СообщитьПользователю("Запрещено проведение документа с датой в будущем времени!",,,,Отказ);
	КонецЕсли;
	// >> MRS #HELP1C-12035 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 04.03.25, ()
	// << MRS #ERP-4947 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 11.03.25, ()
	// << MRS #ERP-5376 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 13.08.25, ()
	Если НЕ ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеремещениеТоваровМеждуФилиалами Тогда
		// >> MRS #ERP-5376 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 13.08.25, ()
		ОргСклада = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СкладПолучатель, "ПЛ_Организация");
	...
	//<< шевалдин 15012020
	Если День(Дата)<>День(Дата+1) Тогда
		Дата=Дата-1;
	КонецЕсли;
	//>>	
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// << MRS #VINE-219 ВикторовАГ <Artem.Viktorov@mriyaresort.com> 25.11.25, (Запрет проведения при недостатке марок Доработка документа "Перемещение товаров")
	MRS_Сервер.ОбработкаПроведенияПеремещениеТоваров(ЭтотОбъект, Отказ);
	// >> MRS #VINE-219 ВикторовАГ <Artem.Viktorov@mriyaresort.com> 25.11.25, (Запрет проведения при недостатке марок Доработка документа "Перемещение товаров")
	...
	// перенос фунциональности расширения MRIAY 
	// После 
	ПЛ_Сервер.IT_ОбработкаПроведения(ЭтотОбъект, Отказ, РежимПроведения);
	//>> Портал-Юг, Кондрашихин, 2024.09.13  
	
	// << MRS # ВикторовАГ <Artem.Viktorov@mriyaresort.com> 10.04.25, (ДокументАкцизныеМарки)
	//MRS_УчетАкцизныхМарокСервер.ЗаписьПроведениеДокумента(ЭтотОбъект, Отказ, РежимЗаписиДокумента.Проведение);
	// >> MRS # ВикторовАГ <Artem.Viktorov@mriyaresort.com> 10.04.25, (ДокументАкцизныеМарки)
	// << MRS #ERP-4591 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 20.05.25, ()
	Если НЕ Отказ И РежимПроведения = РежимПроведенияДокумента.Неоперативный Тогда
		СписокСкладовУведомления = MRS_СлужебныеПараметрыПовтИсп.ПолучитьЗначение("СписокСкладовПолучателейУведомленияОПеремещении","СписокЗначений");
		НайденныйСклад = СписокСкладовУведомления.НайтиПоЗначению(СкладПолучатель);
		Если НЕ НайденныйСклад = Неопределено Тогда
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	// << MRS # ВикторовАГ <Artem.Viktorov@mriyaresort.com> 10.04.25, (ДокументАкцизныеМарки)
	//MRS_УчетАкцизныхМарокСервер.ЗаписьПроведениеДокумента(ЭтотОбъект, Отказ, РежимЗаписиДокумента.ОтменаПроведения)
	// >> MRS # ВикторовАГ <Artem.Viktorov@mriyaresort.com> 10.04.25, (ДокументАкцизныеМарки)
...
Процедура ПриЗаписи(Отказ)
	...
	// << MRS # ВикторовАГ <Artem.Viktorov@mriyaresort.com> 10.04.25, (ДокументАкцизныеМарки)
	//MRS_УчетАкцизныхМарокСервер.ЗаписьПроведениеДокумента(ЭтотОбъект, Отказ, РежимЗаписиДокумента.Запись)
	// >> MRS # ВикторовАГ <Artem.Viktorov@mriyaresort.com> 10.04.25, (ДокументАкцизныеМарки)
...
Процедура ОтправитьУведомление()
	...
	ТекстЗапроса = "
	|ВЫБРАТЬ
	|	MRS_СвязиОбъектовСистемы.Значение_2 КАК Ответственный
	|ИЗ
	|	РегистрСведений.MRS_СвязиОбъектовСистемы КАК MRS_СвязиОбъектовСистемы
	|ГДЕ
	|	MRS_СвязиОбъектовСистемы.Значение_1 = &Значение_1
	|	И НЕ MRS_СвязиОбъектовСистемы.Значение_2.Недействителен
	|";
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("Значение_1", СкладПолучатель);
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		ОтправитьПисьмоОтвественному(Ссылка, Выборка.Ответственный);
	КонецЦикла;
	
	
КонецПроцедуры

Процедура ОтправитьПисьмоОтвественному(ПеремещениеТоваровДок, Ответственный)
	
	ОбъектПеремещениеТоваровДок = ПеремещениеТоваровДок.ПолучитьОбъект();  
	НавигационнаяСсылка = ПолучитьНавигационнуюСсылку(ОбъектПеремещениеТоваровДок);  
	ОсновнаяПочта = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(Ответственный,Справочники.ВидыКонтактнойИнформации.EmailПользователя,ТекущаяДата(),Истина);
	
	ТемаСообщения = "Новое перемещение товаров. " + ПеремещениеТоваровДок;  
	ТекстСообщения = "Создано перемещение товаров в 1С:ERP. " + Символы.ПС + ПеремещениеТоваровДок + "." + 
	Символы.ПС + "Ссылка на документ: "  + НавигационнаяСсылка + Символы.ПС;
	СодержаниеТовары = "Поступило на склад: " + СкладПолучатель.Наименование + ":" + Символы.ПС;
	Для Каждого Строка Из Товары Цикл
		СодержаниеТовары = СодержаниеТовары + "Номенклатура - " + Строка.Номенклатура + " в количестве - " + Строка.Количество + " " + Строка.Номенклатура.ЕдиницаИзмерения + Символы.ПС;
	КонецЦикла;
	ТекстСообщения = ТекстСообщения + СодержаниеТовары + Символы.ПС + "Ответственный - " + Ответственный;
		
	УспешностьОтправки = Ложь;
	Письмо = Новый ИнтернетПочтовоеСообщение;
	Письмо.Тема = ТемаСообщения;
	Текст = Письмо.Тексты.Добавить(ТекстСообщения, ТипТекстаПочтовогоСообщения.ПростойТекст);  
	Письмо.Отправитель = "erp@mriyaresort.com";
	Письмо.ИмяОтправителя = "Mriya Resort & Spa";
	Письмо.Получатели.Добавить(ОсновнаяПочта);
	...
	Профиль.Пароль = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(УчетнаяЗапись);
	...
	Попытка
		...
		//Сообщение.Текст = "Подключение к почтовому серверу Mriya Resort & Spa успешно установлено.";
		Если НЕ УспешностьОтправки Тогда
			Если (Письмо.ОбратныйАдрес.Количество() > 0) Тогда 
				Для Каждого АдресДляОтвета из Письмо.ОбратныйАдрес Цикл
					Письмо.Получатели.Добавить(АдресДляОтвета.Адрес);
				КонецЦикла;
			Иначе
				Письмо.Получатели.Добавить(Письмо.Отправитель.Адрес);
			КонецЕсли;
		КонецЕсли;    
		Почта.Послать(Письмо);
		//Сообщение.Текст = Сообщение.Текст + Символы.ПС + "Письмо отправлено ответственному по складу, по адресу - " + ОсновнаяПочта + ".";
		Почта.Отключиться();
		УспешностьОтправки = Истина;
	Исключение
		УспешностьОтправки = Ложь;
		//Сообщение.Текст = "Не удалось отправить письмо поставщику по данному адресу - " + ОсновнаяПочта + ". " + "Проверьте на корректность электронный адрес поставщика!" + 
		//Символы.ПС + "Не удалось подключиться к почтовому серверу Mriya Resort & Spa - " + ОписаниеОшибки();
	КонецПопытки;
	//ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение.Текст, ПеремещениеТоваровДок);
