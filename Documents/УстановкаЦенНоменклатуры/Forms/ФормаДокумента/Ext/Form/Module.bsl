...
//>>Швецов. ERP-5481. Интегарция приказа в елма
&НаКлиенте
Перем ИгнорироватьПроверкуЦен; // Флаг игнорирования проверки цен ниже себестоимости при повторной записи
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//>>Швецов. ERP-5481. Интегарция приказа в елма
	// Обработка параметра с незаписанным документом из временного хранилища
	Если Параметры.Свойство("АдресДокументаВХранилище") 
		И ЗначениеЗаполнено(Параметры.АдресДокументаВХранилище) Тогда
		
		XMLСтрока = ПолучитьИзВременногоХранилища(Параметры.АдресДокументаВХранилище);
		Если ТипЗнч(XMLСтрока) = Тип("Строка") Тогда
			
			// Десериализуем документ из XML
			ЧтениеXML = Новый ЧтениеXML;
			ЧтениеXML.УстановитьСтроку(XMLСтрока);
			ДокументИзХранилища = СериализаторXDTO.ПрочитатьXML(ЧтениеXML);
			ЧтениеXML.Закрыть();
			
			Если ТипЗнч(ДокументИзХранилища) = Тип("ДокументОбъект.УстановкаЦенНоменклатуры") Тогда
				ЗначениеВРеквизитФормы(ДокументИзХранилища, "Объект");
			КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//>>Швецов. ERP-5481. Интеграция приказа в Елма
	// Проверка цен ниже себестоимости (если флаг игнорирования не установлен)
	Если ТребуетсяОтправкаВЕЛМАСервер() Тогда
		Если НЕ Отказ И НЕ ИгнорироватьПроверкуЦен Тогда
			
			РезультатПроверки = ПроверитьЦеныНижеКостаНаСервере();
			
			Если РезультатПроверки.НайденыОтклонения Тогда
				Объект.Согласован = Ложь;
				Объект.Статус = Перечисления.СтатусыУстановокЦенНоменклатуры.НеСогласован;
				// Блокируем запись и показываем диалог пользователю
				Отказ = Истина;
				ПоказатьДиалогСогласованияЦен(РезультатПроверки.СписокПозиций);
			Иначе
				Объект.Согласован = Истина;
				Объект.Статус = Перечисления.СтатусыУстановокЦенНоменклатуры.Согласован;
				ПоказатьОповещениеПользователя("Согласование документа", , "Все цены в документе согласованы автоматически!", БиблиотекаКартинок.Информация32);
			КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	//>> Швецов. ERP-5481. 19.11.25. Интеграция с ЕЛМА
	...
	Если MRS_ИнтеграцияЕЛМАСервер.ТребуетсяОтправкаВЕЛМА(ТекущийОбъект) Тогда
		...
		Если СтатусДокумента = Неопределено 
			ИЛИ СтатусДокумента.Статус = Перечисления.MRS_СтатусыСинхронизацииELMA.Отклонен Тогда
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ТребуетсяОткрытьФормуПараметровЕЛМА") 
		...
		Результат = ПоставитьПозицииВОчередьSimphony();
		ПоказатьРезультатПостановкиВОчередьSimphony(Результат);
		
		Оповестить("Запись_питМеню", Объект.Ссылка, ЭтаФорма);
	КонецЕсли;
	//<<
...
&НаКлиенте
Процедура Подключаемый_КомандаПечатьПриказаЕЛМА(Команда)
	
	// Вызов клиентского модуля интеграции ЕЛМА
	MRS_ИнтеграцияЕЛМАКлиент.КомандаПечатьПриказаЕЛМА(ЭтотОбъект);
...
&НаКлиенте
Процедура Подключаемый_ОткрытьURLProcess(Команда)
		
	// Открытие ссылка на процесс ЕЛМА
	Если ЗначениеЗаполнено(ЭтотОбъект.URLProcess) Тогда
		ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку(ЭтотОбъект.URLProcess);	
	КонецЕсли;	
...
Процедура ОткрытьФормуПараметровПриказаЕЛМА()
	...
	ПараметрыФормы.Вставить("ВидЦены", Объект.ВидыЦен[0].ВидЦены);
...
&НаСервере
Функция ТребуетсяОтправкаВЕЛМАСервер()
	
	Возврат MRS_ИнтеграцияЕЛМАСервер.ТребуетсяОтправкаВЕЛМА(Объект);
	
КонецФункции

&НаСервере
Функция ПоставитьПозицииВОчередьSimphony()
	
	Возврат MRS_ИнтеграцияSimphonyСервер.ПоставитьВОчередьВыгрузкиНаСервереПриИзмененииЦен(Объект);
	
КонецФункции

&НаКлиенте
Процедура ПоказатьРезультатПостановкиВОчередьSimphony(Результат)
	
	Если Результат.Успешно Тогда
		
		Если Результат.КоличествоПозиций > 0 Тогда
			
			ТекстСообщения = СтрШаблон("В очередь выгрузки Simphony добавлено позиций: %1", Результат.КоличествоПозиций);
			ПоказатьОповещениеПользователя("Постановка в очередь", , ТекстСообщения);
...
#Область ПроверкаЦенНижеСебестоимости

// Серверная функция-обертка для проверки цен ниже себестоимости
//
// Возвращаемое значение:
//  Структура - результат проверки:
//   * НайденыОтклонения - Булево
//   * СписокПозиций - Массив из Структура
//
&НаСервере
Функция ПроверитьЦеныНижеКостаНаСервере()
	
	Возврат MRS_ИнтеграцияЕЛМАСервер.ПроверитьЦеныНижеКоста(Объект);
	
КонецФункции

// Показывает диалог согласования цен с детализацией позиций
//
// Параметры:
//  СписокПозиций - Массив из Структура - список позиций с отклонениями
//
&НаКлиенте
Процедура ПоказатьДиалогСогласованияЦен(СписокПозиций)
	
	// Формируем текст сообщения с детализацией
	ТекстСообщения = "Обнаружены цены ниже себестоимости:" + Символы.ПС + Символы.ПС;
	...
	Для Каждого Позиция Из СписокПозиций Цикл
		...
		ТекстСообщения = ТекстСообщения + 
			СтрШаблон("%1. %2", Счетчик, Строка(Позиция.Номенклатура)) + Символы.ПС +
			СтрШаблон("   Установленная цена: %1 руб.", Формат(Позиция.УстановленнаяЦена, "ЧЦ=15; ЧДЦ=2; ЧН=0,00")) + Символы.ПС +
			СтрШаблон("   Расчетная себестоимость: %1 руб.", Формат(Позиция.РасчетнаяЦена, "ЧЦ=15; ЧДЦ=2; ЧН=0,00")) + Символы.ПС +
			СтрШаблон("   Отклонение: %1 руб. (%2%%)", 
				Формат(Позиция.Отклонение, "ЧЦ=15; ЧДЦ=2; ЧН=0,00"), 
				Формат(Позиция.ОтклонениеПроцент, "ЧЦ=5; ЧДЦ=2; ЧН=0,00")) + Символы.ПС + Символы.ПС;
		
	КонецЦикла;
	
	ТекстСообщения = ТекстСообщения + 
		"Прайс-лист будет отправлен на согласование в систему ЕЛМА." + Символы.ПС + Символы.ПС +
		"Продолжить запись документа?";
	
	// Показываем диалог с вопросом
	ОписаниеОповещения = Новый ОписаниеОповещения("ОбработкаОтветаДиалогаСогласованияЦен", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещения, ТекстСообщения, РежимДиалогаВопрос.ДаНет);
	
КонецПроцедуры

// Обработчик ответа пользователя на диалог согласования цен
//
// Параметры:
...
//
&НаКлиенте
Процедура ОбработкаОтветаДиалогаСогласованияЦен(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		
		// Пользователь подтвердил запись
		// Устанавливаем флаг игнорирования проверки и повторяем запись
		ИгнорироватьПроверкуЦен = Истина;
		
		// Повторная запись документа
		Записать();
		
		// Сбрасываем флаг после записи
		ИгнорироватьПроверкуЦен = Ложь;
		
	Иначе
		
		// Пользователь отказался от записи
		// Ничего не делаем, остаемся в форме документа для корректировки цен
