...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	|	ПРЕДСТАВЛЕНИЕ(АктРец.Номенклатура.ЕдиницаИзмерения) КАК ЕдиницаИзмеренияРецептуры
	|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		НаименованиеРецептуры = Выборка.НаименованиеРецептуры + " , ед.изм: " + Выборка.ЕдиницаИзмеренияРецептуры;
		...
		// Временное хранилище строк ингредиентов, чтобы потом заполнить ТаблицаДанных
		ДанныеИнгредиентов = Новый Массив;
		...
		Для Каждого СтрокаДерева Из СтрокиДерева Цикл
			...
			// считаем стоимость материала на норму
		    СтоимостьМатериалаНаНорму = Окр(Цена * НаКоличествоБлюда, 2);

		    ИтогоСумма = ИтогоСумма + СтоимостьМатериалаНаНорму;

		    // Сохраняем данные ингредиента во временный массив
		    Стр = Новый Структура;
		    Стр.Вставить("Материал", Строка(СтрокаДерева.Номенклатура));
		    Стр.Вставить("ЕдиницаИзмерения", Строка(ЕдиницаИзмерения));
		    Стр.Вставить("НормаРасхода", НаКоличествоБлюда);
		    Стр.Вставить("СтоимостьНаНорму", СтоимостьМатериалаНаНорму); // это пойдет в ЦенаМатериала
		    ДанныеИнгредиентов.Добавить(Стр);
		...
		// пишем строки в ТаблицаДанных
		Для Каждого Стр Из ДанныеИнгредиентов Цикл
		    ...
		    НоваяСтрока.Материал = Стр.Материал + ", ед.изм: " + Стр.ЕдиницаИзмерения + ". Норма расхода: " + Стр.НормаРасхода;
		    //НоваяСтрока.ЕдиницаИзмерения = Стр.ЕдиницаИзмерения;
		    НоваяСтрока.НормаРасхода = Стр.НормаРасхода;

		    // ЦенаМатериала = себестоимость на норму расхода (как ты и хочешь)
		    НоваяСтрока.ЦенаМатериала = Стр.СтоимостьНаНорму;

		    // Одинаковая для всех строк рецептуры
		    НоваяСтрока.СебестоимостьЕдиницы = ИтогоСумма;
