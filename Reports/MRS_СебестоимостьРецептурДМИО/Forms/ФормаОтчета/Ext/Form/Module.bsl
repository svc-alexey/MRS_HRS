...
// Выгружает отчет в файл Excel или CSV
//
&НаКлиенте
Процедура ВыгрузитьВФайл(Команда)
	
	// Проверяем, сформирован ли отчет
	Если Элементы.ОтчетТабличныйДокумент.Документ.ВысотаТаблицы = 0 Тогда
		
		ПоказатьПредупреждение(, "Отчет не сформирован. Сначала сформируйте отчет.");
		Возврат;
	...
	ДиалогВыбораФайла.Заголовок = "Выберите файл для сохранения отчета";
	ДиалогВыбораФайла.ПолноеИмяФайла = "Себестоимость_Рецептур_ДМИО";
	
	// Добавляем фильтры форматов
	ДиалогВыбораФайла.Фильтр = "Книга Excel (*.xlsx)|*.xlsx|CSV файл (*.csv)|*.csv";
	ДиалогВыбораФайла.ИндексФильтра = 0;
	
	Оповещение = Новый ОписаниеОповещения("ВыгрузитьВФайлПослеВыбораФайла", ЭтотОбъект);
	ДиалогВыбораФайла.Показать(Оповещение);
	
КонецПроцедуры

// Обработчик выбора файла для выгрузки
//
&НаКлиенте
Процедура ВыгрузитьВФайлПослеВыбораФайла(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	ТабличныйДокумент = Элементы.ОтчетТабличныйДокумент.Документ;
	...
	Попытка
		...
		Если СтрЗаканчиваетсяНа(НРег(ИмяФайла), ".xlsx") Тогда
			...
			ПоказатьОповещениеПользователя("Выгрузка завершена", , "Отчет успешно выгружен в файл: " + ИмяФайла, БиблиотекаКартинок.Успешно32);
			
		ИначеЕсли СтрЗаканчиваетсяНа(НРег(ИмяФайла), ".csv") Тогда
		...
		Иначе
			...
			ПоказатьПредупреждение(, "Неподдерживаемый формат файла. Выберите .xlsx или .csv файл.");
			
		КонецЕсли;
		
	Исключение
		
		ТекстОшибки = "Ошибка при выгрузке отчета: " + ОписаниеОшибки();
...
Процедура СформироватьОтчет()
	...
	//+пит
	питИзменениеТиповыхФормКлиент.ПередФормированиемОчтета(ЭтотОбъект);
	//-пит
