...
Функция ЗапросПоискаЧека(КлючевыеПоляСтрокой) Экспорт
	...
	// Ищем чеки по ключевым полям напрямую в документах, а не через регистр статусов
	// Это предотвращает создание дублей, если запись в регистре статусов отсутствует
	// Приоритет отдаём: 1) проведенным, 2) непомеченным на удаление, 3) помеченным на удаление
...
Функция ПолучитьЧек(Запрос, КлючевыеПоля) Экспорт
	...
	ПоляЗаполняемыеПриЗагрузкеСтрокой =
		"Ответственный,ПЛ_ДатаОткрытияЧека,ПЛ_КодСотрудника,Организация,КассаККМ,Подразделение,ДисконтнаяКарта,ПЛ_КодКарточки";
...
Функция ПолучениеДанныхORACLE(ПараметрыВыборки)
	...
	|    SELECT 
	|        c.checkid,
	|        MAX(c.detailindex) AS detailindex,
