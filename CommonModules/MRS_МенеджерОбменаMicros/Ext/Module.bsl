...
Запрос = Новый Запрос;
	...
	Для Каждого Строка Из ТаблицаЗначенийORACLE Цикл
		...
		Если НЕ Строка.НоменклатураИзМеню.Пустая() Тогда
		...
		Если Строка.АлкогольнаяПродукция И НЕ Строка.НоменклатураАлкоголь.Пустая() Тогда
			Строка.Номенклатура = Строка.НоменклатураАлкоголь;
		ИначеЕсли НЕ Строка.НоменклатураАлкоголь.Пустая() Тогда
			Строка.Номенклатура = Строка.НоменклатураАлкоголь;
		КонецЕсли;
...
Функция ПроверитьОстатокНоменклатуры(Номенклатура, Склад, ДатаПроверки)
	...
	Если Номенклатура.Пустая() Или Склад.Пустая() Тогда
		Возврат Ложь;
	КонецЕсли;
