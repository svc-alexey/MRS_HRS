...
Функция ВыполнитьПолучениеДанныхMicros(ФронтСистема, НачалоПериода, КонецПериода, ПараметрыКасс = Неопределено, НомерЧека = Неопределено) Экспорт
	...
	// Используем новую функцию загрузки через HTTP API
	ТаблицаЗначенийORACLE = ПолучениеДанныхHTTP(ПараметрыВыборки);
...
Процедура ВыполнитьЗагрузкуДанныхMicros(ФронтСистема, НачалоПериода, КонецПериода, ПараметрыКасс = Неопределено, НомерЧека  = Неопределено, Пользователь = Неопределено) Экспорт
	...
	ЗаписьЖурналаРегистрацииОбмена("Загрузка чеков Micros", "HTTP-запрос выполнен за " + (ВремяОкончанияSQL - ВремяНачала) + " сек.");
...
#Область РаботаСHTTPAPI

// Загрузка данных и получение таблицы значений через HTTP API
//
// Параметры:
//  ПараметрыВыборки - Структура - параметры выборки с полями:
//   * НачалоПериода - Дата - начало периода
//   * КонецПериода - Дата - конец периода
//   * СтрокаВыборкиТочекПродаж - Строка - коды точек продаж через запятую
//   * ФронтСистема - ПеречислениеСсылка.ПЛ_ТипыФронтСистем
//   * НомерЧека - Число - номер конкретного чека (0 для загрузки всех)
//   * ИДЧека - Число - ID чека в Micros
//
// Возвращаемое значение:
//  ТаблицаЗначений - таблица чеков с заполненными реквизитами
//
Функция ПолучениеДанныхHTTP(ПараметрыВыборки)
	
	ТаблицаЗначенийHTTP = Новый ТаблицаЗначений;
	
	// Определяем, какие точки продаж нужно загружать
	МассивТочекПродаж = Новый Массив;
	
	Если ЗначениеЗаполнено(ПараметрыВыборки.СтрокаВыборкиТочекПродаж) Тогда
		
		// Разбиваем строку кодов точек на массив
		СтрокаТочек = СтрЗаменить(ПараметрыВыборки.СтрокаВыборкиТочекПродаж, " ", "");
		МассивСтрок = СтрРазделить(СтрокаТочек, ",");
		
		Для Каждого КодСтрока Из МассивСтрок Цикл
			
			Попытка
				КодЧисло = Число(КодСтрока);
				МассивТочекПродаж.Добавить(КодЧисло);
			Исключение
	...
	Иначе
		...
		// Если точки не указаны, загружаем все доступные точки продаж из регистра сведений
		Запрос = Новый Запрос;
		Запрос.Текст = 
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		Пока Выборка.Следующий() Цикл
			...
			Попытка
				КодЧисло = Число(Выборка.КодТочкиПродаж);
				МассивТочекПродаж.Добавить(КодЧисло);
			Исключение
				// Пропускаем некорректные значения
			КонецПопытки;
		...
		Если МассивТочекПродаж.Количество() = 0 Тогда
			ЗаписьЖурналаРегистрацииОбмена("Загрузка чеков Micros", "Не найдены точки продаж в регистре сведений ПЛ_СоответствиеСФронтСистемами", Истина);
			Возврат ТаблицаЗначенийHTTP;
		КонецЕсли;
		
	КонецЕсли;
	
	// Если указан конкретный номер чека, используем отдельный эндпоинт
	Если ЗначениеЗаполнено(ПараметрыВыборки.НомерЧека) И ПараметрыВыборки.НомерЧека <> 0 Тогда
		
		ТаблицаЧека = ПолучитьДанныеЧекаПоНомеруHTTP(ПараметрыВыборки.НомерЧека);
		
		Если ТаблицаЧека <> Неопределено Тогда
			ТаблицаЗначенийHTTP = ТаблицаЧека;
		КонецЕсли;
		
	Иначе
		
		// Загружаем по периоду для каждой точки продаж
		НачалоПериода = НачалоДня(ПараметрыВыборки.НачалоПериода);
		КонецПериода = КонецДня(ПараметрыВыборки.КонецПериода);
		
		// Проходим по каждому дню в периоде
		ТекущаяДата = НачалоПериода;
		
		Пока НачалоДня(ТекущаяДата) <= НачалоДня(КонецПериода) Цикл
			
			// Пропускаем текущий день и будущие даты
			Если НачалоДня(ТекущаяДата) >= НачалоДня(ТекущаяДатаСеанса()) Тогда
				ТекущаяДата = ТекущаяДата + 86400; // Следующий день
				Продолжить;
			КонецЕсли;
			
			Для Каждого КодТочкиПродаж Из МассивТочекПродаж Цикл
				
				// Загружаем чеки за день для точки продаж
				ТаблицаДня = ПолучитьДетализациюЧековПоПериодуHTTP(КодТочкиПродаж, ТекущаяДата);
				
				Если ТаблицаДня <> Неопределено И ТаблицаДня.Количество() > 0 Тогда
					
					// Объединяем с общей таблицей
					Для Каждого СтрокаДня Из ТаблицаДня Цикл
						
						ЗаполнитьЗначенияСвойств(ТаблицаЗначенийHTTP.Добавить(), СтрокаДня);
	...
	Если ТаблицаЗначенийHTTP.Количество() = 0 Тогда
	...
	// Теперь выполняем сопоставление с объектами 1С (аналогично Oracle версии)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ТаблицаHTTP.ДатаЗакрытияЧека КАК ДатаЗакрытияЧека,
	               |	ТаблицаHTTP.ДатаОткрытияЧека КАК ДатаОткрытияЧека,
	               |	ТаблицаHTTP.ДатаГруппировка КАК ДатаГруппировка,
	               |	ТаблицаHTTP.КодТовара КАК КОДТОВАРА,
	               |	ТаблицаHTTP.КодТовараЧислом КАК КОДТОВАРАЧИСЛОМ,
	               |	ВЫБОР
	               |		КОГДА НоменклатураСоответствие.Объект ЕСТЬ NULL
	               |			ТОГДА ТаблицаHTTP.КоличествоТовара
	               |		ИНАЧЕ ВЫБОР
	               |				КОГДА НоменклатураСоответствие.Количество * ТаблицаHTTP.КоличествоТовара = 0
	               |					ТОГДА ТаблицаHTTP.КоличествоТовара
	               |				ИНАЧЕ НоменклатураСоответствие.Количество * ТаблицаHTTP.КоличествоТовара
	               |			КОНЕЦ
	               |	КОНЕЦ КАК КОЛИЧЕСТВОТОВАРА,
	               |	ТаблицаHTTP.НазваниеТовара КАК НАЗВАНИЕТОВАРА,
	               |	ТаблицаHTTP.НаименованиеТочкиПродаж КАК НАИМЕНОВАНИЕТОЧКИПРОДАЖ,
	               |	ТаблицаHTTP.НомерТочкиПродаж КАК НОМЕРТОЧКИПРОДАЖ,
	               |	ТаблицаHTTP.НомерЧека КАК НОМЕРЧЕКА,
	               |	ТаблицаHTTP.СуммаСкидкиЧека КАК СУММАСКИДКИЧЕКА,
	               |	ТаблицаHTTP.СуммаСкидкиСтроки КАК СУММАСКИДКИСТРОКИ,
	               |	ТаблицаHTTP.СуммаТовара КАК СУММАТОВАРА,
	               |	ТаблицаHTTP.СтоимостьЧека КАК СТОИМОСТЬЧЕКА,
	               |	ТаблицаHTTP.ЦенаТовара КАК ЦЕНАТОВАРА,
	               |	ТаблицаHTTP.КодСтавкиНДС КАК КодСтавкиНДС,
	               |	ТаблицаHTTP.СтавкаНДССтрокой КАК СтавкаНДССтрокой,
	               |	ТаблицаHTTP.НаименованиеСкидкиЧека КАК НАИМЕНОВАНИЕСКИДКИЧЕКА,
	               |	ТаблицаHTTP.НомерМетодаОплаты КАК НОМЕРМЕТОДАОПЛАТЫ,
	               |	ТаблицаHTTP.НомерКассы КАК НОМЕРКАССЫ,
	               |	ТаблицаHTTP.НомерПозицииВЧеке КАК НомерПозицииВЧеке,
	               |	Кассы.ФронтСистема КАК ФронтСистема,
	               |	ЕСТЬNULL(Кассы.Объект, ЗНАЧЕНИЕ(Справочник.КассыККМ.ПустаяСсылка)) КАК КассаККМ,
	               |	ЕСТЬNULL(Кассы.Объект.Владелец, ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)) КАК Организация,
	               |	ЕСТЬNULL(Кассы.Объект.Склад, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)) КАК Склад,
	               ...
	               |	ЕСТЬNULL(НоменклатураСоответствие.НеВлияетНаУчетТМЦ, ЛОЖЬ) КАК НеВлияетНаУчетТМЦ,
	               |	питВидыОплатОрганизаций.Ссылка КАК МетодОплаты,
	               |	ТаблицаHTTP.ИДЧека КАК ИДЧека
	               |ИЗ
	               |	&ТаблицаHTTP КАК ТаблицаHTTP
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПЛ_СоответствиеСФронтСистемами КАК Кассы
	               |		ПО (ПОДСТРОКА(ВЫРАЗИТЬ(ТаблицаHTTP.НомерТочкиПродаж КАК Строка(20, )), 1, 20) = ПОДСТРОКА(Кассы.Код, 1, 20))
	               |			И (Кассы.ФронтСистема = &ФронтСистема)
	               |			И (Кассы.ТипОбъекта = &ТипОбъектаКасса)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПЛ_СоответствиеСФронтСистемами КАК НоменклатураСоответствие
	               |		ПО (ПОДСТРОКА(ТаблицаHTTP.КодТовара, 1, 20) = ПОДСТРОКА(НоменклатураСоответствие.Код, 1, 20))
	               |			И (НоменклатураСоответствие.ФронтСистема = &ФронтСистема)
	               |			И (НоменклатураСоответствие.ТипОбъекта = &ТипОбъектаНоменклатура)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.MRS_МетодыОплатФронтСистем КАК MRS_МетодыОплатФронтСистем
	               |			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.питВидыОплатОрганизаций КАК питВидыОплатОрганизаций
	               |			ПО MRS_МетодыОплатФронтСистем.ТипОплаты = питВидыОплатОрганизаций.ТипОплаты
	               |		ПО ТаблицаHTTP.НомерМетодаОплаты = MRS_МетодыОплатФронтСистем.НомерМетодаОплаты
	               |			И (MRS_МетодыОплатФронтСистем.ФронтСистема = &ФронтСистема)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.питМеню КАК питМеню
	               |		ПО ТаблицаHTTP.КодТовараЧислом = питМеню.MRS_КодSimphony";
	...
	Запрос.УстановитьПараметр("ТипОбъектаНоменклатура", ТипОбъектаНоменклатура);
   	Запрос.УстановитьПараметр("ФронтСистема", ПараметрыВыборки.ФронтСистема);
	Запрос.УстановитьПараметр("ТаблицаHTTP", ТаблицаЗначенийHTTP);
	
	ТаблицаЗначенийHTTP = Запрос.Выполнить().Выгрузить();
	
	// Определяем номенклатуру с учетом остатков на складе (аналогично Oracle версии)
	Для Каждого Строка Из ТаблицаЗначенийHTTP Цикл
	...
	ТаблицаЗначенийHTTP.Сортировать("НОМЕРЧЕКА, НомерПозицииВЧеке");

	Возврат ТаблицаЗначенийHTTP;
	
КонецФункции

// Преобразует JSON ответ от API в таблицу значений чеков
//
// Параметры:
//  ДанныеJSON - Структура, Массив - распарсенный JSON ответ от API
//  ТипЭндпоинта - Строка - тип эндпоинта ("check-details" или "check")
//
// Возвращаемое значение:
//  ТаблицаЗначений - таблица чеков с полями согласно маппингу
//
Функция ПреобразоватьJSONВТаблицуЧеков(ДанныеJSON, ТипЭндпоинта)
	
	// Создаем таблицу значений с нужной структурой колонок
	ТаблицаЧеков = Новый ТаблицаЗначений;
	
	// Добавляем колонки согласно маппингу из PRD
	ТаблицаЧеков.Колонки.Добавить("НомерТочкиПродаж", Новый ОписаниеТипов("Число"));
	ТаблицаЧеков.Колонки.Добавить("НаименованиеТочкиПродаж", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(200)));
	ТаблицаЧеков.Колонки.Добавить("НомерКассы", Новый ОписаниеТипов("Число"));
	ТаблицаЧеков.Колонки.Добавить("НомерЧека", Новый ОписаниеТипов("Число"));
	ТаблицаЧеков.Колонки.Добавить("ИДЧека", Новый ОписаниеТипов("Число"));
	ТаблицаЧеков.Колонки.Добавить("ДатаОткрытияЧекаСтрока", Новый ОписаниеТипов("Строка"));
	ТаблицаЧеков.Колонки.Добавить("ДатаЗакрытияЧекаСтрока", Новый ОписаниеТипов("Строка"));
	ТаблицаЧеков.Колонки.Добавить("КодТовара", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(50)));
	ТаблицаЧеков.Колонки.Добавить("КодТовараЧислом", Новый ОписаниеТипов("Число"));
	ТаблицаЧеков.Колонки.Добавить("НазваниеТовара", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(500)));
	ТаблицаЧеков.Колонки.Добавить("КоличествоТовара", Новый ОписаниеТипов("Число"));
	ТаблицаЧеков.Колонки.Добавить("ЦенаТовара", Новый ОписаниеТипов("Число"));
	ТаблицаЧеков.Колонки.Добавить("СуммаТовара", Новый ОписаниеТипов("Число"));
	ТаблицаЧеков.Колонки.Добавить("СтоимостьЧекаБезСкидки", Новый ОписаниеТипов("Число"));
	ТаблицаЧеков.Колонки.Добавить("СуммаСкидкиЧека", Новый ОписаниеТипов("Число"));
	ТаблицаЧеков.Колонки.Добавить("СтоимостьЧека", Новый ОписаниеТипов("Число"));
	ТаблицаЧеков.Колонки.Добавить("СуммаСкидкиСтроки", Новый ОписаниеТипов("Число"));
	ТаблицаЧеков.Колонки.Добавить("КодСтавкиНДС", Новый ОписаниеТипов("Число"));
	ТаблицаЧеков.Колонки.Добавить("СтавкаНДССтрокой", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(200)));
	ТаблицаЧеков.Колонки.Добавить("НаименованиеСкидкиЧека", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(200)));
	ТаблицаЧеков.Колонки.Добавить("НомерМетодаОплаты", Новый ОписаниеТипов("Число"));
	ТаблицаЧеков.Колонки.Добавить("ИмяСотрудника", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(200)));
	ТаблицаЧеков.Колонки.Добавить("НомерПозицииВЧеке", Новый ОписаниеТипов("Число"));
	ТаблицаЧеков.Колонки.Добавить("НомерСтрокиСкидки", Новый ОписаниеТипов("Число"));
	
	// Добавляем колонки для дат
	ТаблицаЧеков.Колонки.Добавить("ДатаГруппировка", Новый ОписаниеТипов("Дата"));
	ТаблицаЧеков.Колонки.Добавить("ДатаОткрытияЧека", Новый ОписаниеТипов("Дата"));
	ТаблицаЧеков.Колонки.Добавить("ДатаЗакрытияЧека", Новый ОписаниеТипов("Дата"));
	
	// Проверка типа эндпоинта и заполнение таблицы
	Если ТипЭндпоинта = "check-details" Тогда
		
		// Обработка ответа от /checks/check-details
		Если ТипЗнч(ДанныеJSON) = Тип("Структура") И ДанныеJSON.Свойство("data") Тогда
			
			МассивДанных = ДанныеJSON.data;
			
			Если ТипЗнч(МассивДанных) = Тип("Массив") Тогда
				
				Для Каждого ГруппаКассира Из МассивДанных Цикл
					
					Если ТипЗнч(ГруппаКассира) = Тип("Структура") И ГруппаКассира.Свойство("items") Тогда
						
						МассивСтрок = ГруппаКассира.items;
						
						Если ТипЗнч(МассивСтрок) = Тип("Массив") Тогда
							
							Для Каждого СтрокаJSON Из МассивСтрок Цикл
								
								ЗаполнитьСтрокуЧекаИзJSON(ТаблицаЧеков, СтрокаJSON);
	...
	ИначеЕсли ТипЭндпоинта = "check" Тогда
		
		// Обработка ответа от /checks/check
		Если ТипЗнч(ДанныеJSON) = Тип("Структура") И ДанныеJSON.Свойство("data") Тогда
			
			ДанныеЧека = ДанныеJSON.data;
			
			Если ТипЗнч(ДанныеЧека) = Тип("Структура") И ДанныеЧека.Свойство("items") Тогда
				
				МассивСтрок = ДанныеЧека.items;
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон("Неизвестный тип эндпоинта: %1", ТипЭндпоинта);
...
// Заполняет строку таблицы чеков из JSON объекта
//
// Параметры:
//  ТаблицаЧеков - ТаблицаЗначений - таблица для заполнения
//  СтрокаJSON - Структура - JSON объект с данными строки чека
//
Процедура ЗаполнитьСтрокуЧекаИзJSON(ТаблицаЧеков, СтрокаJSON)
	
	НоваяСтрока = ТаблицаЧеков.Добавить();
	
	// Заполняем поля согласно маппингу из PRD
	Если СтрокаJSON.Свойство("rvcId") Тогда
		НоваяСтрока.НомерТочкиПродаж = СтрокаJSON.rvcId;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("rvcName") Тогда
		НоваяСтрока.НаименованиеТочкиПродаж = СтрокаJSON.rvcName;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("cashierNumber") Тогда
		НоваяСтрока.НомерКассы = СтрокаJSON.cashierNumber;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("checkNumber") Тогда
		НоваяСтрока.НомерЧека = СтрокаJSON.checkNumber;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("checkId") Тогда
		НоваяСтрока.ИДЧека = СтрокаJSON.checkId;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("checkOpenDate") Тогда
		НоваяСтрока.ДатаОткрытияЧекаСтрока = СтрокаJSON.checkOpenDate;
		// Преобразование строки даты в тип Дата
		НоваяСтрока.ДатаОткрытияЧека = СтроковыеФункцииКлиентСервер.СтрокаВДату(СтрокаJSON.checkOpenDate, ЧастиДаты.ДатаВремя);
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("checkCloseDate") Тогда
		НоваяСтрока.ДатаЗакрытияЧекаСтрока = СтрокаJSON.checkCloseDate;
		// Преобразование строки даты в тип Дата
		НоваяСтрока.ДатаЗакрытияЧека = СтроковыеФункцииКлиентСервер.СтрокаВДату(СтрокаJSON.checkCloseDate, ЧастиДаты.ДатаВремя);
		НоваяСтрока.ДатаГруппировка = НачалоДня(НоваяСтрока.ДатаЗакрытияЧека);
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("productCode") Тогда
		НоваяСтрока.КодТовара = СтрокаJSON.productCode;
		// Преобразование кода товара в число
		Попытка
			НоваяСтрока.КодТовараЧислом = Число(СтрокаJSON.productCode);
		Исключение
			НоваяСтрока.КодТовараЧислом = 0;
		КонецПопытки;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("productName") Тогда
		НоваяСтрока.НазваниеТовара = СтрокаJSON.productName;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("productCount") Тогда
		НоваяСтрока.КоличествоТовара = СтрокаJSON.productCount;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("price") Тогда
		НоваяСтрока.ЦенаТовара = СтрокаJSON.price;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("sum") Тогда
		НоваяСтрока.СуммаТовара = СтрокаJSON.sum;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("sumWithoutDiscount") Тогда
		НоваяСтрока.СтоимостьЧекаБезСкидки = СтрокаJSON.sumWithoutDiscount;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("discountSum") Тогда
		НоваяСтрока.СуммаСкидкиЧека = СтрокаJSON.discountSum;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("total") Тогда
		НоваяСтрока.СтоимостьЧека = СтрокаJSON.total;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("stringDiscount") Тогда
		НоваяСтрока.СуммаСкидкиСтроки = СтрокаJSON.stringDiscount;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("codeOfTaxRate") Тогда
		НоваяСтрока.КодСтавкиНДС = СтрокаJSON.codeOfTaxRate;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("taxRateString") Тогда
		НоваяСтрока.СтавкаНДССтрокой = СтрокаJSON.taxRateString;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("nameOfDiscount") Тогда
		НоваяСтрока.НаименованиеСкидкиЧека = СтрокаJSON.nameOfDiscount;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("numberOfPayType") Тогда
		НоваяСтрока.НомерМетодаОплаты = СтрокаJSON.numberOfPayType;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("employerName") Тогда
		НоваяСтрока.ИмяСотрудника = СтрокаJSON.employerName;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("positionNumber") Тогда
		НоваяСтрока.НомерПозицииВЧеке = СтрокаJSON.positionNumber;
	КонецЕсли;
	
	Если СтрокаJSON.Свойство("numberOfDiscountString") Тогда
		НоваяСтрока.НомерСтрокиСкидки = СтрокаJSON.numberOfDiscountString;
	КонецЕсли;
	
КонецПроцедуры

// Получает детализацию чеков по периоду и точке продаж через HTTP API
//
// Параметры:
//  КодТочкиПродаж - Число - ID Revenue Center (точки продаж)
//  Дата - Дата - дата загрузки чеков
//
// Возвращаемое значение:
//  ТаблицаЗначений - таблица чеков или Неопределено при ошибке
//
Функция ПолучитьДетализациюЧековПоПериодуHTTP(КодТочкиПродаж, Дата)
	
	// Получение URL из константы
	URLСервиса = Константы.MRS_URLСервисаSimphony.Получить();
	
	Если ПустаяСтрока(URLСервиса) Тогда
		ЗаписьЖурналаРегистрацииОбмена("Загрузка чеков Micros", "Не установлен URL сервиса в константе MRS_URLСервисаSimphony", Истина);
		Возврат Неопределено;
	КонецЕсли;
	...
	Endpoint = СтрШаблон("/checks/check-details?rvcId=%1&date=%2", КодТочкиПродаж, ДатаСтрокой);
	
	Попытка
		
		// Отправка HTTP запроса через функцию из MRS_ИнтеграцияSimphonyСервер
		HTTPОтвет = MRS_ИнтеграцияSimphonyСервер.ОтправитьЗапросAPI("GET", Endpoint, "", Заголовки);
		
		// Обработка ответа
		Если HTTPОтвет.КодСостояния = 200 Тогда
			
			// Успех - парсим JSON
			ОтветJSON = MRS_ИнтеграцияSimphonyСервер.ПарсироватьОтветJSON(HTTPОтвет);
			
			// Преобразование JSON в таблицу значений
			ТаблицаЧеков = ПреобразоватьJSONВТаблицуЧеков(ОтветJSON, "check-details");
		...
		ИначеЕсли HTTPОтвет.КодСостояния = 400 Тогда
			...
			// Ошибка параметров
			СообщениеОбОшибке = СтрШаблон("Ошибка HTTP 400: Неверные параметры запроса. rvcId=%1, date=%2", КодТочкиПродаж, ДатаСтрокой);
			ЗаписьЖурналаРегистрацииОбмена("Загрузка чеков Micros", СообщениеОбОшибке, Истина);
			Возврат Новый ТаблицаЗначений; // Возвращаем пустую таблицу
			
		ИначеЕсли HTTPОтвет.КодСостояния = 404 Тогда
			
			// Данные не найдены
			СообщениеОбОшибке = СтрШаблон("HTTP 404: Данные не найдены за период. rvcId=%1, date=%2", КодТочкиПродаж, ДатаСтрокой);
			ЗаписьЖурналаРегистрацииОбмена("Загрузка чеков Micros", СообщениеОбОшибке);
			Возврат Новый ТаблицаЗначений; // Возвращаем пустую таблицу
			
		ИначеЕсли HTTPОтвет.КодСостояния = 500 Тогда
			
			// Ошибка сервера
			СообщениеОбОшибке = СтрШаблон("Ошибка HTTP 500: Внутренняя ошибка сервера. rvcId=%1, date=%2", КодТочкиПродаж, ДатаСтрокой);
			ЗаписьЖурналаРегистрацииОбмена("Загрузка чеков Micros", СообщениеОбОшибке, Истина);
			Возврат Неопределено;
		...
		Иначе
			...
			// Прочие ошибки
			СообщениеОбОшибке = СтрШаблон("Ошибка HTTP %1: %2", HTTPОтвет.КодСостояния, HTTPОтвет.ПолучитьТелоКакСтроку());
			ЗаписьЖурналаРегистрацииОбмена("Загрузка чеков Micros", СообщениеОбОшибке, Истина);
			Возврат Неопределено;
	...
	Исключение
		...
		СообщениеОбОшибке = СтрШаблон("Ошибка загрузки чеков Micros через HTTP API:%1Endpoint: %2%1Параметры: rvcId=%3, date=%4%1Описание: %5",
			Символы.ПС, Endpoint, КодТочкиПродаж, ДатаСтрокой, ТекстОшибки);
		ЗаписьЖурналаРегистрацииОбмена("Загрузка чеков Micros", СообщениеОбОшибке, Истина);
		Возврат Неопределено;
...
// Получает данные чека по номеру через HTTP API
//
// Параметры:
//  НомерЧека - Число, Строка - номер кассового чека
//
// Возвращаемое значение:
//  ТаблицаЗначений - таблица с детализацией чека или Неопределено при ошибке
//
Функция ПолучитьДанныеЧекаПоНомеруHTTP(НомерЧека)
	...
	// Формирование endpoint
	Endpoint = СтрШаблон("/checks/check?checkNumber=%1", НомерЧека);
	...
	Попытка
		...
		Если HTTPОтвет.КодСостояния = 200 Тогда
			...
			// Преобразование JSON в таблицу значений
			ТаблицаЧеков = ПреобразоватьJSONВТаблицуЧеков(ОтветJSON, "check");
		...
		ИначеЕсли HTTPОтвет.КодСостояния = 400 Тогда
			...
			// Ошибка параметров
			СообщениеОбОшибке = СтрШаблон("Ошибка HTTP 400: Неверный номер чека. checkNumber=%1", НомерЧека);
			ЗаписьЖурналаРегистрацииОбмена("Загрузка чеков Micros", СообщениеОбОшибке, Истина);
		...
		ИначеЕсли HTTPОтвет.КодСостояния = 404 Тогда
			...
			// Чек не найден
			СообщениеОбОшибке = СтрШаблон("HTTP 404: Чек не найден. checkNumber=%1", НомерЧека);
			ЗаписьЖурналаРегистрацииОбмена("Загрузка чеков Micros", СообщениеОбОшибке);
			Возврат Новый ТаблицаЗначений;
		...
		ИначеЕсли HTTPОтвет.КодСостояния = 500 Тогда
			...
			// Ошибка сервера
			СообщениеОбОшибке = СтрШаблон("Ошибка HTTP 500: Внутренняя ошибка сервера. checkNumber=%1", НомерЧека);
			ЗаписьЖурналаРегистрацииОбмена("Загрузка чеков Micros", СообщениеОбОшибке, Истина);
			Возврат Неопределено;
	...
	Исключение
		...
		// Таймаут или ошибка соединения
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		СообщениеОбОшибке = СтрШаблон("Ошибка загрузки чека Micros через HTTP API:%1Endpoint: %2%1Параметры: checkNumber=%3%1Описание: %4",
			Символы.ПС, Endpoint, НомерЧека, ТекстОшибки);
		ЗаписьЖурналаРегистрацииОбмена("Загрузка чеков Micros", СообщениеОбОшибке, Истина);
		Возврат Неопределено;
