...
Процедура докПриобретениеТоваровУслугПослеЗаписиНаСервере(Форма, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ИспользоватьПеремещениеПоНесколькимЗаказам Тогда
		...
		Для каждого элСклад Из МассивСкладов Цикл
				...
				//ПараметрыОткрытия = СозданиеНаОснованииУТВызовСервера.ПараметрыОткрытияФормыНакладнойНаОснованииЗаказа(ТекущийОбъект.Ссылка, "ПеремещениеТоваров");
				ДанныеЗаполнения = Новый Структура("Основание, Склад"); 
				ДанныеЗаполнения.Основание = ТекущийОбъект.Ссылка;
				ДанныеЗаполнения.Склад = ТекущийОбъект.Склад;
			ДокПеремещение = Документы.ПеремещениеТоваров.СоздатьДокумент();
			ДокПеремещение.Дата = ТекущаяДатаСеанса();
			ДокПеремещение.Заполнить(ДанныеЗаполнения); 
		...
		// Удаляем/корректируем строки, которые уже были распределены в предыдущие перемещения
		КоличествоСтрокПеремещения = ДокПеремещение.Товары.Количество();
		Для Индекс = 1 По КоличествоСтрокПеремещения Цикл
			ОбратныйИндекс = КоличествоСтрокПеремещения - Индекс;
			СтрокаПеремещения = ДокПеремещение.Товары.Получить(ОбратныйИндекс);
			...
			СтрокиПТУ = ТЗ_РаспределеноИзПТУ.НайтиСтроки(СтруктураПоиска);
			
			КоличествоОстаток = 0;
			Для Каждого СтрокаПТУ Из СтрокиПТУ Цикл
				КоличествоОстаток = КоличествоОстаток + (СтрокаПТУ.Количество - СтрокаПТУ.РаспределенноеКоличество);
			КонецЦикла;
			
			Если КоличествоОстаток <= 0 Тогда
				// Все строки ПТУ полностью распределены - удаляем
				ДокПеремещение.Товары.Удалить(ОбратныйИндекс);
			ИначеЕсли КоличествоОстаток < СтрокаПеремещения.Количество Тогда
				// Остаток меньше - корректируем количество
				СтрокаПеремещения.Количество = КоличествоОстаток;
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого СтрокаПеремещения Из ДокПеремещение.Товары Цикл
			// Ищем соответствующую строку в ПТУ
			СтруктураПоиска = Новый Структура("Номенклатура, Характеристика, Назначение");
			ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаПеремещения);
			
			СтрокиПТУ = ТЗ_РаспределеноИзПТУ.НайтиСтроки(СтруктураПоиска);
			
			// Записываем распределённое количество ТОЛЬКО для строк, привязанных к заказам
			Если ЗначениеЗаполнено(СтрокаПеремещения.ЗаказНаПеремещение) Тогда
				КоличествоДляРаспределения = СтрокаПеремещения.Количество;
