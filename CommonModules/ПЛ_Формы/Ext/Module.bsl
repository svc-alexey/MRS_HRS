...
Процедура докПриобретениеТоваровУслугПослеЗаписиНаСервере(Форма, ТекущийОбъект, ПараметрыЗаписи)
	...
	// Простая таблица: сколько из каждой строки ПТУ уже распределено
	...
	// Заполняем табличную часть: добавляем строки из ПТУ с остатками
	Для Каждого СтрокаПТУТаблица Из ТЗ_СтрокиПТУ Цикл
		ОстатокКоличества = СтрокаПТУТаблица.КоличествоВсего - СтрокаПТУТаблица.КоличествоИспользовано;
		
		Если ОстатокКоличества > 0 Тогда
			// Берем исходную строку из ПТУ
			СтрокаПТУИсходная = ТекущийОбъект.Товары[СтрокаПТУТаблица.НомерСтроки];
			
			// Добавляем в перемещение
	...
	// После привязки суммируем количество по номенклатуре (т.к. могли разбиться строки)
	ТЗ_ИтоговоеКоличество = Новый ТаблицаЗначений;
	ТЗ_ИтоговоеКоличество.Колонки.Добавить("Номенклатура");
	ТЗ_ИтоговоеКоличество.Колонки.Добавить("Характеристика");
	ТЗ_ИтоговоеКоличество.Колонки.Добавить("Назначение");
	ТЗ_ИтоговоеКоличество.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число"));
	...
	Для Каждого СтрокаПеремещения Из ДокПеремещение.Товары Цикл
		...
		СтрокиИтого = ТЗ_ИтоговоеКоличество.НайтиСтроки(СтруктураПоиска);
		Если СтрокиИтого.Количество() = 0 Тогда
			НоваяСтрока = ТЗ_ИтоговоеКоличество.Добавить();
		...
		Иначе
			СтрокиИтого[0].Количество = СтрокиИтого[0].Количество + СтрокаПеремещения.Количество;
		КонецЕсли;
	КонецЦикла;
	
	// Обновляем таблицу: списываем использованное количество с первых строк ПТУ
	Для Каждого СтрокаИтого Из ТЗ_ИтоговоеКоличество Цикл
		КоличествоКСписанию = СтрокаИтого.Количество;
		...
		Для Каждого СтрокаПТУТаблица Из ТЗ_СтрокиПТУ Цикл
			...
			Если СтрокаПТУТаблица.Номенклатура = СтрокаИтого.Номенклатура
				И СтрокаПТУТаблица.Характеристика = СтрокаИтого.Характеристика
				И СтрокаПТУТаблица.Назначение = СтрокаИтого.Назначение Тогда
				
				ОстатокВСтроке = СтрокаПТУТаблица.КоличествоВсего - СтрокаПТУТаблица.КоличествоИспользовано;
				
				Если ОстатокВСтроке > 0 Тогда
					Если ОстатокВСтроке >= КоличествоКСписанию Тогда
						// Хватает в этой строке
						СтрокаПТУТаблица.КоличествоИспользовано = СтрокаПТУТаблица.КоличествоИспользовано + КоличествоКСписанию;
						КоличествоКСписанию = 0;
					Иначе
						// Берем сколько есть
						СтрокаПТУТаблица.КоличествоИспользовано = СтрокаПТУТаблица.КоличествоВсего;
						КоличествоКСписанию = КоличествоКСписанию - ОстатокВСтроке;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
