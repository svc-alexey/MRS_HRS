	...
	Попытка
		...
		Если ИспользоватьПеремещениеПоНесколькимЗаказам Тогда
			...
			Для каждого элСклад Из МассивСкладов Цикл
				...
				//<< Портал-Юг, Баринов, 2019.08.28 [5.2.1 Автоматическое создание документа «перемещение товаров» на основании документа поступления]
				// Если перемещение делается в одном направлении, то и количество сверх заказа перемещаем туда же
				Если МассивСкладов.Количество() > 1 Тогда
				//>> Портал-Юг, Баринов, 2019.08.28
				
					Для каждого стрБезЗаказа из СтрокиБезЗаказа Цикл
						флагЕстьПозицииСверхЗаказа = Истина;
						// Записать в ТЧ и потом эти строки загружать в новые перемещения
						// 
				...
				//<< Портал-Юг, Баринов, 2019.08.28 [5.2.1 Автоматическое создание документа «перемещение товаров» на основании документа поступления]
				Иначе
					// Типовая в релизе 2.4.8.57 требует заполненный заказ даже для строк сверх заказа
					Если МассивЗаказовНаПеремещение.Количество() Тогда
