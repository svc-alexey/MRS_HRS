...
Процедура докПриобретениеТоваровУслугПослеЗаписиНаСервере(Форма, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ИспользоватьПеремещениеПоНесколькимЗаказам Тогда
		...
		// Ищем склад без организации среди складов заказов для распределения излишков
		СкладБезОрганизации = Неопределено;
		...
		Для каждого элСклад Из МассивСкладов Цикл
			...
			Если СтрокиБезЗаказа.Количество() > 0 Тогда
				...
				Если МассивЗаказовНаПеремещение.Количество() = 1 Тогда
					// Один заказ - оставляем излишки в этом перемещении, привязываем к заказу
				...
				ИначеЕсли МассивЗаказовНаПеремещение.Количество() > 1 Тогда
					// Несколько заказов - определяем склад для излишков
					...
					Если НЕ ЗначениеЗаполнено(ОргТекущегоСклада) Тогда
						...
						ПоследнийЗаказ = МассивЗаказовНаПеремещение[МассивЗаказовНаПеремещение.Количество() - 1];
						Для Каждого стрБезЗаказа Из СтрокиБезЗаказа Цикл
							стрБезЗаказа.ЗаказНаПеремещение = ПоследнийЗаказ;
						КонецЦикла;
					Иначе
