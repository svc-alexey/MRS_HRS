...
Процедура докПриобретениеТоваровУслугПослеЗаписиНаСервере(Форма, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ИспользоватьПеремещениеПоНесколькимЗаказам Тогда
		...
		Для Каждого СтрокаПТУИсходная Из ТекущийОбъект.Товары Цикл
			...
			ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаПТУИсходная);
			
			СтрокиРаспределения = ТЗ_РаспределеноИзПТУ.НайтиСтроки(СтруктураПоиска);
