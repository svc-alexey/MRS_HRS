...
Процедура докПриобретениеТоваровУслугПослеЗаписиНаСервере(Форма, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ИспользоватьПеремещениеПоНесколькимЗаказам Тогда
		...
		// Удаляем строки из ПТУ, которые уже были распределены в предыдущие перемещения
		// И корректируем количество для частично распределённых строк
		...
		Для Индекс = 1 По КоличествоСтрокПеремещения Цикл
			...
			СтрокаПеремещения = ДокПеремещение.Товары[ОбратныйИндекс];
				
				// Ищем соответствующую строку в таблице распределения
				...
				КоличествоДляПеремещения = СтрокаПеремещения.Количество;
				...
				Для Каждого СтрокаПТУ Из СтрокиПТУ Цикл
					...
					Если ОстатокВСтрокеПТУ > 0 Тогда
						// Есть нераспределённое количество в этой строке ПТУ
						Если ОстатокВСтрокеПТУ >= КоличествоДляПеремещения Тогда
							// Достаточно для полного обеспечения
							СтрокаПТУ.РаспределенноеКоличество = СтрокаПТУ.РаспределенноеКоличество + КоличествоДляПеремещения;
							УдалитьСтроку = Ложь;
							Прервать;
						Иначе
							// Берём сколько есть
							СтрокаПеремещения.Количество = ОстатокВСтрокеПТУ;
