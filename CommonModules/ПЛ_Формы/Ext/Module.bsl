...
Процедура докПриобретениеТоваровУслугПослеЗаписиНаСервере(Форма, ТекущийОбъект, ПараметрыЗаписи)
	...
	Попытка
		...
		//ПТиУ_Товары = ТекущийОбъект.Товары.Выгрузить();
	...
	Если ИспользоватьПеремещениеПоНесколькимЗаказам Тогда
		флагЕстьПозицииСверхЗаказа = Ложь;
		
	// Таблица для учёта распределённого количества из ПТУ
	ТЗ_РаспределеноИзПТУ = ТекущийОбъект.Товары.Выгрузить();
	ТЗ_РаспределеноИзПТУ.Колонки.Добавить("РаспределенноеКоличество", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 3)));
	ТЗ_РаспределеноИзПТУ.Колонки.Добавить("НомерСтрокиПТУ", Новый ОписаниеТипов("Число"));
	
	НомерСтроки = 0;
	Для Каждого Строка Из ТЗ_РаспределеноИзПТУ Цикл
		Строка.РаспределенноеКоличество = 0;
		Строка.НомерСтрокиПТУ = НомерСтроки;
	...
	КонецЦикла;
		...
		ДокПеремещение.ЗаказНаПеремещение = МассивЗаказовНаПеремещение[0].Ссылка;
		
	// Заполняем табличную часть вручную только нераспределенными остатками из ПТУ
	// Создаём временную таблицу для хранения связи строк перемещения с исходными строками ПТУ
	ТЗ_СвязьСтрокПеремещения = Новый ТаблицаЗначений;
	ТЗ_СвязьСтрокПеремещения.Колонки.Добавить("Номенклатура");
	ТЗ_СвязьСтрокПеремещения.Колонки.Добавить("Характеристика");
	ТЗ_СвязьСтрокПеремещения.Колонки.Добавить("Назначение");
	ТЗ_СвязьСтрокПеремещения.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число"));
	ТЗ_СвязьСтрокПеремещения.Колонки.Добавить("НомерСтрокиПТУ", Новый ОписаниеТипов("Число"));
	
	Для НомерСтроки = 0 По ТЗ_РаспределеноИзПТУ.Количество() - 1 Цикл
		СтрокаРаспределения = ТЗ_РаспределеноИзПТУ[НомерСтроки];
		ОстатокКоличества = СтрокаРаспределения.Количество - СтрокаРаспределения.РаспределенноеКоличество;
		
		Если ОстатокКоличества > 0 Тогда
			// Есть нераспределенное количество - добавляем в перемещение
			...
			// Запоминаем связь для последующей записи распределения
			СтрокаСвязи = ТЗ_СвязьСтрокПеремещения.Добавить();
			СтрокаСвязи.Номенклатура = НоваяСтрока.Номенклатура;
			СтрокаСвязи.Характеристика = НоваяСтрока.Характеристика;
			СтрокаСвязи.Назначение = НоваяСтрока.Назначение;
			СтрокаСвязи.Количество = НоваяСтрока.Количество;
			СтрокаСвязи.НомерСтрокиПТУ = НомерСтроки;
		КонецЕсли;
	КонецЦикла;
			
				//<< Портал-Юг, Баринов, 2019.08.18 [5.2.1 Автоматическое создание документа «перемещение товаров» на основании документа поступления]
				// Чтобы строки привязались, нужно чтобы в перемещении была та же номенклатура, что и в заказе.
				// После привязки к заказу её нужно будет вернуть обратно
				//<< Портал-Юг, Баринов, 2019.10.28 [5.2.1 Автоматическое создание документа «перемещение товаров» на основании документа поступления]
				ТЗ_НоменклатураПеремещения = Новый ТаблицаЗначений;
				ТЗ_НоменклатураПеремещения.Колонки.Добавить("Номенклатура");
				ТЗ_НоменклатураПеремещения.Колонки.Добавить("Характеристика");
				ТЗ_НоменклатураПеремещения.Колонки.Добавить("Назначение");
				ТЗ_НоменклатураПеремещения.Колонки.Добавить("НоменклатураЗаказа");
				ТЗ_НоменклатураПеремещения.Колонки.Добавить("КоличествоЗаказа");
				ТЗ_НоменклатураПеремещения.Колонки.Добавить("КоэфПересчета");
				//>> Портал-Юг, Баринов, 2019.10.28
				Если ТЗ_ПодменнаяНоменклатура.Количество() Тогда
					Для каждого СтрТовары Из ДокПеремещение.Товары Цикл
						// Номенклатура однозначно определяет номенклатуру заказа и коэффициент.
						// При наличии нескольких строк с одинаковой номенклатурой по разным заказам, номенклатура заказа и коэффициент будут совпадать.
						// Поэтому достаточно найти одну любую строку с номенклатурой.
						СтрокаПодмены = ТЗ_ПодменнаяНоменклатура.Найти(СтрТовары.Номенклатура, "Номенклатура");
						Если СтрокаПодмены <> Неопределено Тогда 
							//<< Портал-Юг, Баринов, 2019.10.28 [5.2.1 Автоматическое создание документа «перемещение товаров» на основании документа поступления]
							стрНоменклатураПеремещения = ТЗ_НоменклатураПеремещения.Добавить();
							ЗаполнитьЗначенияСвойств(стрНоменклатураПеремещения, СтрокаПодмены);
							ЗаполнитьЗначенияСвойств(стрНоменклатураПеремещения, СтрТовары, "Характеристика, Назначение");
							//>> Портал-Юг, Баринов, 2019.10.28
							СтрТовары.Номенклатура = СтрокаПодмены.НоменклатураЗаказа;
							СтрТовары.Количество = СтрТовары.Количество * СтрокаПодмены.КоэфПересчета;							
							//<< Портал-Юг, Баринов, 2019.10.28 [5.2.1 Автоматическое создание документа «перемещение товаров» на основании документа поступления]
							стрНоменклатураПеремещения.КоличествоЗаказа = СтрТовары.Количество;
							//>> Портал-Юг, Баринов, 2019.10.28
						КонецЕсли;	
					КонецЦикла;
				КонецЕсли;
	//>> Портал-Юг, Баринов, 2019.08.18
	ПривязатьСтрокиКЗаказамНаПеремещение(ДокПеремещение, МассивЗаказовНаПеремещение);
	
	// После привязки записываем фактически распределённое количество
	// Суммируем количество по номенклатуре, которое осталось в документе
	ТЗ_КоличествоПоНоменклатуре = Новый ТаблицаЗначений;
	ТЗ_КоличествоПоНоменклатуре.Колонки.Добавить("Номенклатура");
	ТЗ_КоличествоПоНоменклатуре.Колонки.Добавить("Характеристика");
	ТЗ_КоличествоПоНоменклатуре.Колонки.Добавить("Назначение");
	ТЗ_КоличествоПоНоменклатуре.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число"));
	
	Для Каждого СтрокаПеремещения Из ДокПеремещение.Товары Цикл
		СтруктураПоиска = Новый Структура("Номенклатура, Характеристика, Назначение");
		ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаПеремещения);
		
		НайденнаяСтрока = ТЗ_КоличествоПоНоменклатуре.Найти(СтрокаПеремещения.Номенклатура, "Номенклатура");
		
		Если НайденнаяСтрока = Неопределено Тогда
			НоваяСтрока = ТЗ_КоличествоПоНоменклатуре.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаПеремещения, "Номенклатура, Характеристика, Назначение");
			НоваяСтрока.Количество = СтрокаПеремещения.Количество;
		Иначе
			НайденнаяСтрока.Количество = НайденнаяСтрока.Количество + СтрокаПеремещения.Количество;
		КонецЕсли;
	КонецЦикла;
	
	// Теперь распределяем суммарное количество по исходным строкам ПТУ
	Для Каждого СтрокаКоличества Из ТЗ_КоличествоПоНоменклатуре Цикл
		КоличествоДляРаспределения = СтрокаКоличества.Количество;
		
		// Распределяем последовательно по строкам ПТУ с такой номенклатурой
		Для НомерСтроки = 0 По ТЗ_РаспределеноИзПТУ.Количество() - 1 Цикл
			Если КоличествоДляРаспределения <= 0 Тогда
				Прервать;
			КонецЕсли;
			
			СтрокаПТУ = ТЗ_РаспределеноИзПТУ[НомерСтроки];
			
			// Проверяем совпадение номенклатуры
			Если СтрокаПТУ.Номенклатура = СтрокаКоличества.Номенклатура 
				И СтрокаПТУ.Характеристика = СтрокаКоличества.Характеристика
				И СтрокаПТУ.Назначение = СтрокаКоличества.Назначение Тогда
				
				ОстатокВСтрокеПТУ = СтрокаПТУ.Количество - СтрокаПТУ.РаспределенноеКоличество;
				
				Если ОстатокВСтрокеПТУ > 0 Тогда
					Если ОстатокВСтрокеПТУ >= КоличествоДляРаспределения Тогда
						СтрокаПТУ.РаспределенноеКоличество = СтрокаПТУ.РаспределенноеКоличество + КоличествоДляРаспределения;
						КоличествоДляРаспределения = 0;
					Иначе
						СтрокаПТУ.РаспределенноеКоличество = СтрокаПТУ.Количество;
						КоличествоДляРаспределения = КоличествоДляРаспределения - ОстатокВСтрокеПТУ;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
			...
			//<< Портал-Юг, Баринов, 2019.08.18 [5.2.1 Автоматическое создание документа «перемещение товаров» на основании документа поступления]
				Если ТЗ_ПодменнаяНоменклатура.Количество() Тогда
					Для каждого текСтрока Из ДокПеремещение.Товары Цикл
						Если НЕ ЗначениеЗаполнено(текСтрока.ЗаказНаПеремещение) Тогда
							Продолжить;
						КонецЕсли; 
						
						СтруктураПоиска = Новый Структура();
						СтруктураПоиска.Вставить("ЗаказНаПеремещение", текСтрока.ЗаказНаПеремещение);
						...
						Если НайденныеСтроки.Количество() Тогда
							//<< Портал-Юг, Баринов, 2019.10.28 [5.2.1 Автоматическое создание документа «перемещение товаров» на основании документа поступления]
							СтрокаПодмены = НайденныеСтроки[0];
							стрНоменклатураПеремещения = ТЗ_НоменклатураПеремещения.Добавить();
							ЗаполнитьЗначенияСвойств(стрНоменклатураПеремещения, СтрокаПодмены);
							ЗаполнитьЗначенияСвойств(стрНоменклатураПеремещения, текСтрока, "Характеристика, Назначение");
							стрНоменклатураПеремещения.КоличествоЗаказа = - текСтрока.Количество;
							//>> Портал-Юг, Баринов, 2019.10.28
				...
				Если МассивСкладов.Количество() = 1 Тогда
					// Иначе строки без заказа всё равно будут удалены
					ТЗ_НоменклатураПеремещения.Свернуть("Номенклатура, Характеристика, Назначение, НоменклатураЗаказа, КоэфПересчета", "КоличествоЗаказа");
					// Для строк, оставшихся без заказа нужно вернуть номенклатуру на ту, что попала в документ из поступления.
					СтруктураПоиска = Новый Структура("Номенклатура, Характеристика, Назначение");
					СтруктураПоиска.Вставить("ЗаказНаПеремещение", Документы.ЗаказНаПеремещение.ПустаяСсылка());
					
					Для каждого СтрокаНоменклатураПеремещения Из ТЗ_НоменклатураПеремещения Цикл
						Если СтрокаНоменклатураПеремещения.КоличествоЗаказа = 0 Тогда 
							Продолжить;
						КонецЕсли;							
						ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаНоменклатураПеремещения);
						СтруктураПоиска.Номенклатура = СтрокаНоменклатураПеремещения.НоменклатураЗаказа;
						НайденныеСтроки = ДокПеремещение.Товары.НайтиСтроки(СтруктураПоиска);
						ИндСтроки = 0;
						Пока ИндСтроки < НайденныеСтроки.Количество() И СтрокаНоменклатураПеремещения.КоличествоЗаказа > 0 Цикл
							НайденнаяСтрока = НайденныеСтроки[ИндСтроки];
							КоличествоЗаменяем = Мин(НайденнаяСтрока.Количество, СтрокаНоменклатураПеремещения.КоличествоЗаказа);
							Если КоличествоЗаменяем < НайденнаяСтрока.Количество Тогда 
								// Нужно разбить строку.
								Если ИндСтроки + 1 < НайденныеСтроки.Количество() Тогда
									//Есть ещё подходящая строка, можем перенести количество в неё
									ПодходящаяСтрока = НайденныеСтроки[ИндСтроки + 1];
									ПодходящаяСтрока.Количество = ПодходящаяСтрока.Количество + НайденнаяСтрока.Количество - КоличествоЗаменяем;
								Иначе
									// Нет больше подходящих строк, оставляем строку как есть. Вдруг поступила именно эта номенклатура, хотя такое крайне маловероятно.
									НоваяСтрока = ДокПеремещение.Товары.Добавить();
									ЗаполнитьЗначенияСвойств(НоваяСтрока, НайденнаяСтрока, , "НомерСтроки");
									НоваяСтрока.Количество = НайденнаяСтрока.Количество - КоличествоЗаменяем;
								КонецЕсли;									
							КонецЕсли;	
							НайденнаяСтрока.Количество = КоличествоЗаменяем / СтрокаНоменклатураПеремещения.КоэфПересчета;
							НайденнаяСтрока.Номенклатура = СтрокаНоменклатураПеремещения.Номенклатура;
							СтрокаНоменклатураПеремещения.КоличествоЗаказа = СтрокаНоменклатураПеремещения.КоличествоЗаказа - КоличествоЗаменяем;
				...
				//<< Портал-Юг, Баринов, 2019.08.18 [5.2.1 Автоматическое создание документа «перемещение товаров» на основании документа поступления]
				// Перенёс ниже, после корректировки ТЧ Товары и Серии
		...
		// Обработка строк сверх заказа (излишков товара)
		СтруктураПоиска = Новый Структура("ЗаказНаПеремещение", Документы.ЗаказНаПеремещение.ПустаяСсылка());
		СтрокиБезЗаказа = ДокПеремещение.Товары.НайтиСтроки(СтруктураПоиска);
		
		Если СтрокиБезЗаказа.Количество() > 0 Тогда
			
			Если МассивСкладов.Количество() = 1 Тогда
				// Один склад - оставляем излишки в этом перемещении, привязываем к заказу
				ЛюбойЗаказНаПеремещение = МассивЗаказовНаПеремещение[0];
				Для Каждого стрБезЗаказа Из СтрокиБезЗаказа Цикл
					стрБезЗаказа.ЗаказНаПеремещение = ЛюбойЗаказНаПеремещение;
				КонецЦикла;
				
			ИначеЕсли МассивСкладов.Количество() > 1 Тогда
				// Несколько складов - определяем куда отправить излишки
				
				// Ищем склад текущего перемещения в массиве складов
				ТекущийСклад = ДокПеремещение.СкладПолучатель;
				ОргТекущегоСклада = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущийСклад, "ПЛ_Организация");
				
				Если НЕ ЗначениеЗаполнено(ОргТекущегоСклада) Тогда
					// Текущее перемещение идет на склад без организации - оставляем излишки здесь
					// Привязываем к первому заказу (как и в шапке документа)
					ПервыйЗаказ = МассивЗаказовНаПеремещение[0];
					Для Каждого стрБезЗаказа Из СтрокиБезЗаказа Цикл
						стрБезЗаказа.ЗаказНаПеремещение = ПервыйЗаказ;
					КонецЦикла;
				Иначе
					// Текущий склад привязан к организации - удаляем излишки, они уйдут в другое перемещение
					флагЕстьПозицииСверхЗаказа = Истина;
					Для Каждого стрБезЗаказа Из СтрокиБезЗаказа Цикл
						ДокПеремещение.Товары.Удалить(стрБезЗаказа);
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
		...
		КонецЕсли;
				...
				//<< Портал-Юг, Баринов, 2019.08.18 [5.2.1 Автоматическое создание документа «перемещение товаров» на основании документа поступления]
				// Если всё количество номенклатуры с сериями попало в одно перемещение, то серии в него же целиком должны попасть.
				// Иначе очищаем серии в перемещении, т.к. неизвестно какая серия из поступления в какое перемещение должна попасть.
				...
				Если ИндексСтрокиСерии > 0 Тогда 
					...
					Для каждого КлючСерий Из КлючиСерий Цикл
						...
						Если СтрокиТоваров.Количество() = 1 И СтрокиТоваров[0].Количество = 0 Тогда 
							// Строки товаров с таким ключём есть, количество совпадает с перемещением.
							// Соответствующие ключу серии остаются в документе
							Продолжить;
						КонецЕсли;
						// удаляем соответствующие ключу серии
						...
						КонецЦикла; 
					КонецЦикла;					
				КонецЕсли; // есть строки в ТЧ Серии перемещения
				//>> Портал-Юг, Баринов, 2019.08.18
				
				//<< Портал-Юг, Баринов, 2019.08.18 [6.2.8. Регистрация документов для выгрузки в БП]
				// Перенёс из строк выше
				ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(НоменклатураСервер.ПараметрыУказанияСерий(ДокПеремещение, Документы.ПеремещениеТоваров));
			...
			//<< Портал-Юг, Баринов, 2019.08.18 [5.2.1 Автоматическое создание документа «перемещение товаров» на основании документа поступления]
			// Серии в перемещение попадают при формировании его на основании поступления типовым механизмом
			// Копировать их дополнительно не нужно.
			//Если МассивСкладов.Количество() = 1 И МассивПеремещений.Количество() = 1 И НЕ флагЕстьПозицииСверхЗаказа Тогда
			//	МассивПеремещений[0].Серии.Загрузить(ТекущийОбъект.Серии.Выгрузить());
			//КонецЕсли;
		    //>> Портал-Юг, Баринов, 2019.08.18
		...
		Иначе	
			Для каждого элЗаказПеремещение Из СоответствиеЗаказовПеремещения Цикл
				...
				Если ДокПеремещение.Товары.Количество() Тогда
					...
					КонецЦикла;
					//<< Портал-Юг, Свинцов И.И., 2019.07.25 [5.2.1 Автоматическое создание документа «перемещение товаров» на основании документа поступления]  
					ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(НоменклатураСервер.ПараметрыУказанияСерий(ДокПеремещение, Документы.ПеремещениеТоваров));
					НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(ДокПеремещение, ПараметрыУказанияСерий);
					//>> Портал-Юг, Свинцов И.И., 2019.07.25
	...
	// проверим остатки по заказу и создадим задачу на закрытие
	//УстановитьПривилегированныйРежим(Истина);
	//СоздаватьЗадачу=Константы.ПЛ_СоздаватьЗадачуНаЗакрытиеЗаказаПоставщику.Получить();
	//УстановитьПривилегированныйРежим(Ложь);
	//Если СоздаватьЗадачу Тогда
	//	ПроверитьОстаткиЗаказовПоставщикам( ТекущийОбъект);
	//КонецЕсли;
	//<< Портал-Юг, Гладкая  Н., 18.11.2020, [SD33228
	
	//<< Портал-Юг, Гладкая  Н., 28.01.2021, [SD33228 Закрытие заказов с высоким процентом поставки ("недовоза")]
	...
	КонецЕсли;
	//<< Портал-Юг, Гладкая  Н., 28.01.2021, [SD33228
