...
// Формирует печатную форму приказа ЕЛМА из пользовательского макета Word
//
// Параметры:
//  СсылкаДокумента - ДокументСсылка.УстановкаЦенНоменклатуры - документ для печати
//
// Возвращаемое значение:
//  Строка - адрес временного хранилища с файлом Word
//
Функция СформироватьПриказЕЛМА(СсылкаДокумента) Экспорт
	
	Попытка
		
		// 1. Получаем макет печати
		// Функция УправлениеПечатью.МакетПечатнойФормы автоматически проверяет 
		// наличие пользовательского макета в РС.ПользовательскиеМакетыПечати
		// и возвращает его, если он есть, иначе берет из метаданных
		...
		Исключение
			// Если макет не найден в метаданных, пробуем получить из пользовательских макетов вручную
			...
			Если МакетДвоичныеДанные = Неопределено Тогда
				ВызватьИсключение "Макет печати ""ПриказЕЛМА"" не найден. " + 
					"Создайте пользовательский макет Word в РС.ПользовательскиеМакетыПечати " + 
					"с объектом ""Документ.УстановкаЦенНоменклатуры"" и именем ""ПриказЕЛМА"" " + 
					"или добавьте макет ПриказЕЛМА в документ УстановкаЦенНоменклатуры.";
			КонецЕсли;
			
		КонецПопытки;
		
		// 2. Инициализируем макет офисного документа
		Макет = УправлениеПечатью.ИнициализироватьМакетОфисногоДокумента(
			МакетДвоичныеДанные, Неопределено);
		
		// 3. Описание областей макета (должны быть настроены в Word-макете)
		...
		// 6. Заполняем шапку документа
		// Теги в макете должны соответствовать именам параметров
		ДанныеШапки = Новый Структура;
		ДанныеШапки.Вставить("Организация", Организация);
		ДанныеШапки.Вставить("СсылкаДата", "[Ссылка.Дата]");
		ДанныеШапки.Вставить("НомерПриказа", "[Ссылка.Номер]");
		ДанныеШапки.Вставить("TopicOfDecree", "[TopicOfDecree]");
		
		// Дата начала действия из параметров приказа
		ИмяПараметраДатаДействия = "ДатаНачалаДействия";
		Если ПараметрыПриказа <> Неопределено И ПараметрыПриказа.Свойство(ИмяПараметраДатаДействия) Тогда
			ДанныеШапки.Вставить(ИмяПараметраДатаДействия, Формат(ПараметрыПриказа.ДатаНачалаДействия, "ДЛФ=D"));
		Иначе
			ДанныеШапки.Вставить(ИмяПараметраДатаДействия, "");
		КонецЕсли;
		
		ОбластьШапки = УправлениеПечатью.ОбластьМакета(Макет, ОписаниеОбластей["Шапка"]);
		УправлениеПечатью.ПрисоединитьОбластьИЗаполнитьПараметры(
			ПечатнаяФорма, ОбластьШапки, ДанныеШапки);
		
		// 7. Заполняем табличную часть
		// Подготавливаем массив структур для коллекции (ПрисоединитьИЗаполнитьКоллекцию требует Массив)
		МассивСтрок = Новый Массив;
		
		Для Каждого СтрокаТовары Из ОбъектДок.Товары Цикл
			
			// Получаем данные для строки таблицы
			ДанныеСтроки = Новый Структура;
			
			// Точка продажи (из вида цены)
			ДанныеСтроки.Вставить("ТочкаПродажи", "[ТочкаПродажи]");
			
			// Название блюда/товара (для ЧЕКА и КНОПКИ на экране Micros)
			// Получаем позицию меню из справочника питМеню
			...
			// Ставка НДС
			ДанныеСтроки.Вставить("СтавкаНДС", "[НДС]");
			
			МассивСтрок.Добавить(ДанныеСтроки);
			
		КонецЦикла;
		
		// Выводим строки в макет
		ОбластьСтроки = УправлениеПечатью.ОбластьМакета(
			Макет, ОписаниеОбластей["СтрокаТаблицы"]);
		
		// Заполняем коллекцию строк (передаем Массив структур)
		УправлениеПечатью.ПрисоединитьИЗаполнитьКоллекцию(
			ПечатнаяФорма, ОбластьСтроки, МассивСтрок);
		
		// 8. Формируем файл и помещаем во временное хранилище
	...
	Исключение
		...
		// Возвращаем пустой адрес при ошибке
		Возврат "";
