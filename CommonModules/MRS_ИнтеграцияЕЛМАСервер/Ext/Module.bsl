...
//  ДокументСсылка - ДокументСсылка.УстановкаЦенНоменклатуры
//
Процедура ДокументУстановкаЦенПередУдалением(ДокументСсылка) Экспорт
	...
	Попытка
		...
		Если ПустаяСтрока(УИДЕЛМА) Тогда
			...
			ЛокальнаяОтменаПриказа(ДокументСсылка, "Документ локально отменен в 1С (УИДЕЛМА отсутствует)");
			Возврат;
		...
		Если НЕ РезультатАвторизации.Успех Тогда
			...
			ЛокальнаяОтменаПриказа(ДокументСсылка, ТекстОшибки, Истина);
			Возврат;
		...
		Если РезультатОтмены.Успех Тогда
			...
			ОбновитьСтатусДокументаПриОтмене(ДокументСсылка, Перечисления.MRS_СтатусыСинхронизацииELMA.Отменен, "");
			ЗаписатьЛог(СтрШаблон("Документ %1 успешно отменен в ЕЛМА и 1С", ДокументСсылка), 
				УровеньЖурналаРегистрации.Информация);
...
//
Процедура ЛокальнаяОтменаПриказа(ДокументСсылка, ТекстСообщения, ТребуетВнимания = Ложь)
	...
	ОбновитьСтатусДокументаПриОтмене(ДокументСсылка, НовыйСтатус, ТекстСообщения);
	
	УровеньЛога = ?(ТребуетВнимания, УровеньЖурналаРегистрации.Предупреждение, УровеньЖурналаРегистрации.Информация);
	ЗаписатьЛог(СтрШаблон("Документ %1: %2", ДокументСсылка, ТекстСообщения), УровеньЛога);
...
//
Процедура ОбновитьСтатусДокументаПриОтмене(ДокументСсылка, НовыйСтатус, ТекстОшибки)
...
//
Функция ПроверитьЦеныНижеКоста(ДокументСсылка) Экспорт
