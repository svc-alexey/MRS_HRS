...
Функция СформироватьПриказЕЛМА(СсылкаДокумента) Экспорт
	...
	Попытка
		...
		Макет = УправлениеПечатью.ИнициализироватьМакетОфисногоДокумента(МакетДвоичныеДанные, Неопределено);
		
		// 3. Инициализация печатной формы
		ПечатнаяФорма = УправлениеПечатью.ИнициализироватьПечатнуюФорму(Неопределено, Неопределено, Макет);
		
		// 4. Получаем объект документа и параметры
		...
		// 5. Подготавливаем параметры шапки (будут добавлены в каждую строку таблицы)
		ДанныеШапки = Новый Структура;
		ДанныеШапки.Вставить("Организация", Организация);
		ДанныеШапки.Вставить("СсылкаДата", Формат(ОбъектДок.Дата, "ДЛФ=D"));
		ДанныеШапки.Вставить("НомерПриказа", ОбъектДок.Номер);
		ДанныеШапки.Вставить("TopicOfDecree", "Об установке цен");
		
		// Дата начала действия
		Если ПараметрыПриказа <> Неопределено И ПараметрыПриказа.Свойство("ДатаНачалаДействия") Тогда
			ДанныеШапки.Вставить("ДатаНачалаДействия", Формат(ПараметрыПриказа.ДатаНачалаДействия, "ДЛФ=D"));
		Иначе
			ДанныеШапки.Вставить("ДатаНачалаДействия", "");
		КонецЕсли;
		
		// 6. Заполняем табличную часть
		// Собираем данные таблицы в строку (для параметра ТабличнаяЧасть)
		ТекстТаблицы = "";
		Для Каждого СтрокаТовары Из ОбъектДок.Товары Цикл
			...
			Если ПозицияМеню <> Неопределено Тогда
				НаименованиеТовара = Строка(ПозицияМеню);
			Иначе
				НаименованиеТовара = Строка(СтрокаТовары.Номенклатура);
			КонецЕсли;
			
			// Формируем строку таблицы (можно адаптировать формат под ваш шаблон)
			СтрокаТаблицы = НаименованиеТовара + " | " + 
				Формат(СтрокаТовары.Цена, "ЧЦ=15; ЧДЦ=2; ЧН=0,00") + Символы.ПС;
			
			ТекстТаблицы = ТекстТаблицы + СтрокаТаблицы;
			
		КонецЦикла;
		
		// Добавляем текст таблицы в параметры шапки
		ДанныеШапки.Вставить("ТабличнаяЧасть", ТекстТаблицы);
		
		// 7. Заполняем все параметры в документе
		// Получаем весь документ как область
		ОписаниеОбластей = Новый Структура;
		
		Попытка
			// Пытаемся найти область "СтрокаТаблицы" для построчного вывода
			...
			Если ОбластьСтроки <> Неопределено Тогда
				// Есть закладка - выводим построчно
				Для Каждого СтрокаТовары Из ОбъектДок.Товары Цикл
					ДанныеСтроки = Новый Структура(ДанныеШапки);
					...
					УправлениеПечатью.ПрисоединитьОбластьИЗаполнитьПараметры(ПечатнаяФорма, ОбластьСтроки, ДанныеСтроки);
				КонецЦикла;
			Иначе
				// Нет закладки - заполняем параметры напрямую в структуре документа
				УправлениеПечатью.ЗаполнитьПараметры(ПечатнаяФорма.СтруктураДокумента, ДанныеШапки);
			КонецЕсли;
			
		Исключение
			// Если закладка не найдена - заполняем параметры напрямую
			УправлениеПечатью.ЗаполнитьПараметры(ПечатнаяФорма.СтруктураДокумента, ДанныеШапки);
		КонецПопытки;
		
		// 7. Формируем файл и получаем двоичные данные
