...
Функция СформироватьПриказЕЛМА(СсылкаДокумента) Экспорт
	...
	Попытка
		...
		// 5. Заполняем параметры шапки (теги {v8 Параметр} в документе)
		Данные = Новый Структура;
		Данные.Вставить("Организация", Организация);
		Данные.Вставить("СсылкаДата", Формат(ОбъектДок.Дата, "ДЛФ=D"));
		Данные.Вставить("НомерПриказа", ОбъектДок.Номер);
		Данные.Вставить("TopicOfDecree", "Об установке цен");
		
		// Дата начала действия
		Если ПараметрыПриказа <> Неопределено И ПараметрыПриказа.Свойство("ДатаНачалаДействия") Тогда
			Данные.Вставить("ДатаНачалаДействия", Формат(ПараметрыПриказа.ДатаНачалаДействия, "ДЛФ=D"));
		Иначе
			Данные.Вставить("ДатаНачалаДействия", "");
		КонецЕсли;
		
		// Заполняем все теги {v8 ...} в документе
		УправлениеПечатью.ЗаполнитьПараметры(ПечатнаяФорма, Данные);
		
		// 6. Заполняем табличную часть (требуется закладка "СтрокаТаблицы" в Word)
