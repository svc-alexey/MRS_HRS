...
Функция ОпроситьСтатусыДокументов() Экспорт
	...
	Запрос.Текст = 
		...
		|	И НЕ ПроцессыЕЛМА.Статус В (ЗНАЧЕНИЕ(Перечисление.MRS_СтатусыСинхронизацииELMA.Подписан),
		|		ЗНАЧЕНИЕ(Перечисление.MRS_СтатусыСинхронизацииELMA.Отклонен),
		|		ЗНАЧЕНИЕ(Перечисление.MRS_СтатусыСинхронизацииELMA.Отменен))
