...
Функция СформироватьJSONДляЕЛМА(ДокументСсылка, ПараметрыПриказа) Экспорт
	...
	UidОрганизации = ПолучитьUidОрганизацииИзПланаОбмена(ВидЦены); //получить из параметра приказа
	...
	Организация = ПолучитьОрганизациюИзПланаОбмена(ВидЦены);  //получить из параметра приказа

	//НаименованиеОрганизации = Организация.Наименование; TODO   убрать заглушки
	НаименованиеОрганизации = Организация;  //получить из параметра приказа

	
	// Формирование DecreeHeadline
	DecreeHeadline = СтрШаблон("Об установке цен для %1 с %2", 
		НаименованиеОрганизации, 
		Формат(ПараметрыПриказа.ДатаНачалаДействия, "ДФ=dd.MM.yyyy")); //реквизит тема из параметров приказа
	
	// Формирование DecreeContent
	DecreeContent = СформироватьDecreeContent(ДокументСсылка);
	
	// Формирование Word-файла приказа
	ДвоичныеДанныеWord = СформироватьWordФайлПриказа(ДокументСсылка, ПараметрыПриказа); // должно быть как в MRS_ИнтеграцияЕлмаСервер.СформироватьПриказЕЛМА
	...
	Context.Вставить("approval", "True");
...
Функция ПолучитьUidФизЛицаЕЛМА(ФизЛицоСсылка) Экспорт
	...
	возврат "ffebdda3-047d-11eb-8169-98f2b3347817";
...
Функция ПолучитьUidТекущегоПользователяЕЛМА() Экспорт
	...
	СвСтатус = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("ИдентификаторДляФормул", "ГУИДЗУП");
	...
	Запрос.УстановитьПараметр("Объект", ТекущийПользователь.ФизическоеЛицо);
...
Функция СформироватьПриказЕЛМА(СсылкаДокумента) Экспорт
	...
	Попытка
		...
		ВидЦены = ОбъектДок.ВидыЦен[0].ВидЦены; 
		ПараметрыПриказа = ПолучитьСохраненныеПараметрыПриказа(СсылкаДокумента);
		
		Если ОбъектДок.ВидыЦен.Количество() > 0 Тогда
...
Функция ПреобразоватьПараметрыПриказаДляJSON(ПараметрыПриказа)
	...
	// Преобразуем ОтветственныйЗаИсполнение в строку (UUID)
	Если ПараметрыПриказа.Свойство("ОтветственныйЗаИсполнение") 
		...
		Результат.Вставить("ОтветственныйЗаИсполнение", 
			Строка(ПараметрыПриказа.ОтветственныйЗаИсполнение.УникальныйИдентификатор()));
	...
	// Преобразуем СписокКОзнакомлению (ТаблицаЗначений или Массив) в массив UUID
		...
		Если ТипЗнч(ПараметрыПриказа.СписокКОзнакомлению) = Тип("ТаблицаЗначений") Тогда
			...
			Для Каждого Строка Из ПараметрыПриказа.СписокКОзнакомлению Цикл
				...
				Если Строка.Свойство("ФизическоеЛицо") И ЗначениеЗаполнено(Строка.ФизическоеЛицо) Тогда
					МассивСпискаКОзнакомлению.Добавить(Строка(Строка.ФизическоеЛицо.УникальныйИдентификатор()));
				КонецЕсли;
		...
		ИначеЕсли ТипЗнч(ПараметрыПриказа.СписокКОзнакомлению) = Тип("Массив") Тогда
			...
			Для Каждого Элемент Из ПараметрыПриказа.СписокКОзнакомлению Цикл
				...
				Если Элемент.Свойство("ФизическоеЛицо") И ЗначениеЗаполнено(Элемент.ФизическоеЛицо) Тогда
					МассивСпискаКОзнакомлению.Добавить(Строка(Элемент.ФизическоеЛицо.УникальныйИдентификатор()));
				КонецЕсли;
...
Функция ПреобразоватьПараметрыПриказаИзJSON(СериализованныеПараметры)
	...
	// Преобразуем ОтветственныйЗаИсполнение из UUID строки в ссылку
	Если СериализованныеПараметры.Свойство("ОтветственныйЗаИсполнение") Тогда
		
		UUIDСтрока = СериализованныеПараметры.ОтветственныйЗаИсполнение;
	...
	// Преобразуем СписокКОзнакомлению из массива UUID в таблицу значений
	...
	Если СериализованныеПараметры.Свойство("СписокКОзнакомлению") 
		...
		Для Каждого UUIDСтрока Из СериализованныеПараметры.СписокКОзнакомлению Цикл
			...
			Исключение
				// Пропускаем невалидные UUID - просто не добавляем их в таблицу
				Ссылка = Неопределено;
			КонецПопытки;
...
Функция ПолучитьДанныеПоВидуЦен(ВидЦены) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураВозврата.Вставить("Организация",Выборка.ТочкаПродаж);
		СтруктураВозврата.Вставить("ГуидЕлмаОрганизации",Выборка.ГуидЕлмаОрганизации);
		СтруктураВозврата.Вставить("ГуидЕлмаОрганизации",Выборка.Склад);
	КонецЦикла;
