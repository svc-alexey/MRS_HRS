...
Функция СформироватьПриказЕЛМА(СсылкаДокумента) Экспорт
	...
	Попытка
		...
		// 6. Описание области для таблицы
		ОписаниеОбластей = Новый Структура;
		УправлениеПечатью.ДобавитьОписаниеОбласти(ОписаниеОбластей, "СтрокаТаблицы", "СтрокаТаблицы");
		
		// 7. Заполняем табличную часть (требуется закладка "СтрокаТаблицы" в Word)
		МассивСтрок = Новый Массив;
		
		Для Каждого СтрокаТовары Из ОбъектДок.Товары Цикл
			
			// Копируем параметры шапки в каждую строку (для заполнения тегов {v8...} в документе)
			ДанныеСтроки = Новый Структура(ДанныеШапки);
			
			// Добавляем данные строки таблицы
			ДанныеСтроки.Вставить("ТочкаПродажи", ""); // TODO: получать из вида цены
			...
			ДанныеСтроки.Вставить("СтавкаНДС", ""); // TODO: получать ставку НДС
			
			МассивСтрок.Добавить(ДанныеСтроки);
			
		КонецЦикла;
		
		// Выводим строки в макет (требуется закладка "СтрокаТаблицы" в Word)
		ОбластьСтроки = УправлениеПечатью.ОбластьМакета(Макет, ОписаниеОбластей["СтрокаТаблицы"]);
		
		// Первую строку выводим отдельно, чтобы заполнить параметры шапки в документе
		Если МассивСтрок.Количество() > 0 Тогда
			УправлениеПечатью.ПрисоединитьОбластьИЗаполнитьПараметры(ПечатнаяФорма, ОбластьСтроки, МассивСтрок[0]);
			
			// Остальные строки выводим в цикле
			Для Индекс = 1 По МассивСтрок.Количество() - 1 Цикл
				УправлениеПечатью.ПрисоединитьОбластьИЗаполнитьПараметры(ПечатнаяФорма, ОбластьСтроки, МассивСтрок[Индекс]);
			КонецЦикла;
		КонецЕсли;
