...
Функция ОтправитьДокументВЕЛМАНемедленно(ДокументСсылка, ПараметрыПриказа) Экспорт
	...
	Результат = Новый Структура("Успех, ТекстОшибки, УИДЕЛМА", Ложь, "", "");
	...
	Попытка
		...
		// 3. Отправка документа в ЕЛМА
		РезультатОтправки = ОтправитьВЕЛМА(ДокументСсылка, AuthToken, ПараметрыПриказа);
		
		Если РезультатОтправки.Успех Тогда
			
			// 4. Сохранение результата в регистре
			НаборЗаписей = РегистрыСведений.MRS_ПроцессыELMA.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.УстановкаЦен.Установить(ДокументСсылка);
			
			Запись = НаборЗаписей.Добавить();
			Запись.УстановкаЦен = ДокументСсылка;
			Запись.УИДЕЛМА = РезультатОтправки.УИДЕЛМА;
			Запись.Статус = Перечисления.MRS_СтатусыСинхронизацииELMA.НаСогласовании;
			...
			Запись.ТекстОшибки = АдресПараметров;
			
			НаборЗаписей.Записать();
			
			ЗаписатьЛог(СтрШаблон("Документ %1 успешно отправлен в ЕЛМА. УИД процесса: %2", 
				ДокументСсылка, РезультатОтправки.УИДЕЛМА), УровеньЖурналаРегистрации.Информация);
			
			Результат.Успех = Истина;
			Результат.УИДЕЛМА = РезультатОтправки.УИДЕЛМА;
...
Функция ПоставитьДокументВОчередьЕЛМА(ДокументСсылка, ПараметрыПриказа) Экспорт
	...
	Попытка
		...
		Запись = НаборЗаписей.Добавить();
		Запись.УстановкаЦен = ДокументСсылка;
		Запись.Статус = Перечисления.MRS_СтатусыСинхронизацииELMA.ВОчереди;
