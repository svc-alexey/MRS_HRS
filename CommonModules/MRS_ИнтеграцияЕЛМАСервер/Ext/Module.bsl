...
Функция ПолучитьМакетПриказаЕЛМА()
	...
	Попытка
		ПутьКМакету = "Документ.УстановкаЦенНоменклатуры.ПриказЕЛМА";
		МакетДвоичныеДанные = УправлениеПечатью.МакетПечатнойФормы(ПутьКМакету);
	Исключение
...
Функция ПолучитьПользовательскийМакетПечати(Объект, ИмяМакета)
	...
	Попытка
		...
		// Проверяем, существует ли РС.ПользовательскиеМакетыПечати в конфигурации
		Попытка
			МетаданныеРегистра = Метаданные.РегистрыСведений.Найти("ПользовательскиеМакетыПечати");
		...
		// Формируем полный путь к макету для логирования
		ИдентификаторМакета = Объект + "." + ИмяМакета;
		
		// Ищем макет в РС.ПользовательскиеМакетыПечати
		Запрос = Новый Запрос;
		Запрос.Текст = 
			"ВЫБРАТЬ ПЕРВЫЕ 1
		...
		Если Выборка.Следующий() Тогда
			// Получаем двоичные данные макета из хранилища значений
	...
	Исключение
		...
		ЗаписатьЛог(СтрШаблон("Ошибка получения пользовательского макета %1: %2", 
			ИдентификаторМакета, ОписаниеОшибки()), УровеньЖурналаРегистрации.Ошибка);
		Возврат Неопределено;
