...
Функция СформироватьПриказЕЛМА(СсылкаДокумента) Экспорт
    ...
    // 1. Берём макет из РС.ПользовательскиеМакетыПечати
    // Путь вида: "Документ.<ИмяДокумента>.<ИмяМакета>"
    // ИмяМакета = то, что в колонке "Имя макета" (не представление в дереве!)
    МакетДвоичныеДанные = УправлениеПечатью.МакетПечатнойФормы(
        "Документ.УстановкаЦенНоменклатуры.ПриказЕЛМА");  // подставь своё имя
    ...
    // 3. Описание областей макета
    // В Word ты при настройке макета задавал группы/области:
    //   - например "Общая" для шапки
    //   - "СтрокаТаблицы" для строк табличной части
    ...
    // 5. Шапка: структура с параметрами, имена строго как в макете
    // В Word теги вида {v8 <НомерДокумента>}, {v8 <ДатаДокумента>} и т.п.
    ОбъектДок = СсылкаДокумента.ПолучитьОбъект();

    ДанныеШапки = Новый Структура;
    ДанныеШапки.Вставить("НомерДокумента", ОбъектДок.Номер);
    ДанныеШапки.Вставить("ДатаДокумента", ОбъектДок.Дата);
    // сюда же остальные параметры, которые ты в макете раскидал
    ...
    // 6. Табличная часть: коллекция строк
    // В макете теги должны ссылаться на поля коллекции,
    // например {v8 <Товары.Номенклатура>}, {v8 <Товары.Количество>} и т.п.
    ...
    Для каждого СтрТч Из ОбъектДок.Товары Цикл
    ...
    // Заполняем коллекцию строк
    УправлениеПечатью.ПрисоединитьИЗаполнитьКоллекцию(
        ПечатнаяФорма, ОбластьСтроки, ТаблицаСтрок);

    // 7. Формируем файл и кладём во временное хранилище
