...
Функция ПодготовитьСоответствиеНоменклатураЦена(СтруктураПараметров) Экспорт
	...
	РассчетПоСебестоимостиСтруктура = СтруктураПараметров.РассчетПоСебестоимости; // Швецов. 22.01.26 ERP-5547. Расчет по себестоимости для цен меню 
	...
	// Швецов. 22.01.26 ERP-5547. Расчет по себестоимости для цен меню
	// Себестоимость будем брать по рассчитанной себестоимости вместо цены по типу цен.
	Если РассчетПоСебестоимостиСтруктура <> Неопределено Тогда
		
		// Извлекаем параметры из структуры
		Организация = РассчетПоСебестоимостиСтруктура.Организация;
		Подразделение = РассчетПоСебестоимостиСтруктура.Подразделение;
		Склад = РассчетПоСебестоимостиСтруктура.Склад;
		
		// Устанавливаем параметры для запроса себестоимости
		Запрос.УстановитьПараметр("Организация", Организация);
		Запрос.УстановитьПараметр("Подразделение", Подразделение);
		Запрос.УстановитьПараметр("Склад", Склад);
		
		// Формируем запрос с каскадной логикой поиска себестоимости
		ТекстЗапросаСебестоимости = 
		"// 1. СтоимостьТоваров по складу
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|ИЗ
		|	РегистрСведений.СтоимостьТоваров.СрезПоследних(&Период, АналитикаУчетаНоменклатуры.Номенклатура В (&Номенклатура)
		|		И АналитикаУчетаНоменклатуры.МестоХранения = &Склад
		|		И Организация = &Организация) КАК СтоимостьТоваровСрезПоследних
		|ГДЕ
		|	СтоимостьТоваровСрезПоследних.Стоимость <> 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|// 2. СтоимостьТоваров по подразделению
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|			РегистрСведений.СтоимостьТоваров КАК СтоимостьТоваров
		|		ГДЕ
		|			СтоимостьТоваров.АналитикаУчетаНоменклатуры.Номенклатура В(&Номенклатура)
		|			И СтоимостьТоваров.АналитикаУчетаНоменклатуры.Подразделение = &Подразделение
		|			И СтоимостьТоваров.Организация = &Организация
		|			И СтоимостьТоваров.Период <= &Период
		...
		|ГДЕ
		|	СтоимостьТоваров.АналитикаУчетаНоменклатуры.Подразделение = &Подразделение
		|	И СтоимостьТоваров.Организация = &Организация
		|	И СтоимостьТоваров.Стоимость <> 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|// 3. СтоимостьТоваров по организации
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СтоимостьТоваров.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
		|	СтоимостьТоваров.Стоимость КАК Стоимость
		...
		|			И СтоимостьТоваров.Период = ПоследниеЗаписи.Период
		|ГДЕ
		|	СтоимостьТоваров.Организация = &Организация
		|	И СтоимостьТоваров.Стоимость <> 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|// 4. Закупки текущего месяца по складу
		|ВЫБРАТЬ
		...
		|			И Закупки.Период >= НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
		|			И Закупки.Период < ДОБАВИТЬКДАТЕ(КОНЕЦПЕРИОДА(&Период, МЕСЯЦ), СЕКУНДА, 1)
		|		СГРУППИРОВАТЬ ПО
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|// 5. Закупки текущего месяца по подразделению
		|ВЫБРАТЬ
		|	Закупки.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
		|	Закупки.Стоимость / Закупки.Количество КАК Стоимость
		...
		|			И Закупки.Период = ПоследниеЗакупки.Период
		|ГДЕ
		|	Закупки.Подразделение = &Подразделение
		|	И Закупки.Стоимость <> 0
		|	И Закупки.Количество <> 0
		|	И Закупки.Регистратор ССЫЛКА Документ.ПриобретениеТоваровУслуг
		|	И Закупки.Период >= НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|// 6. Закупки текущего месяца по организации
		|ВЫБРАТЬ
		|	Закупки.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
		|	Закупки.Стоимость / Закупки.Количество КАК Стоимость
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|// 7. Закупки предыдущего месяца по складу
		|ВЫБРАТЬ
		|	Закупки.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
		|	Закупки.Стоимость / Закупки.Количество КАК Стоимость
		...
		|			И Закупки.Количество <> 0
		|			И Закупки.Регистратор ССЫЛКА Документ.ПриобретениеТоваровУслуг
		|			И Закупки.Период >= НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(&Период, МЕСЯЦ, -1), МЕСЯЦ)
		|			И Закупки.Период < ДОБАВИТЬКДАТЕ(КОНЕЦПЕРИОДА(ДОБАВИТЬКДАТЕ(&Период, МЕСЯЦ, -1), МЕСЯЦ), СЕКУНДА, 1)
		|		СГРУППИРОВАТЬ ПО
		|			Закупки.АналитикаУчетаНоменклатуры.Номенклатура) КАК ПоследниеЗакупки
		|		ПО Закупки.АналитикаУчетаНоменклатуры.Номенклатура = ПоследниеЗакупки.Номенклатура
		|			И Закупки.Период = ПоследниеЗакупки.Период
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|// 8. Закупки предыдущего месяца по подразделению
		|ВЫБРАТЬ
		|	Закупки.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
		|	Закупки.Стоимость / Закупки.Количество КАК Стоимость
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|// 9. Закупки предыдущего месяца по организации
		|ВЫБРАТЬ
		|	Закупки.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
		|	Закупки.Стоимость / Закупки.Количество КАК Стоимость
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|// 10. СтоимостьТоваров только по номенклатуре
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СтоимостьТоваров.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
		|	СтоимостьТоваров.Стоимость КАК Стоимость
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|// 11. Закупки текущего месяца только по номенклатуре
		|ВЫБРАТЬ
		|	Закупки.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
		|	Закупки.Стоимость / Закупки.Количество КАК Стоимость
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|// 12. Закупки предыдущего месяца только по номенклатуре
		|ВЫБРАТЬ
		|	Закупки.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
		|	Закупки.Стоимость / Закупки.Количество КАК Стоимость
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|// Каскадная логика выбора себестоимости (13 уровней приоритета)
		|ВЫБРАТЬ
		|	ЦеныНоменклатуры.Номенклатура КАК Номенклатура,
		|	ВЫБОР
		...
		|	ЗНАЧЕНИЕ(Справочник.Валюты.ВалютаРегламентированногоУчета) КАК Валюта,
		|	ЦеныНоменклатуры.Номенклатура.ЕдиницаИзмерения КАК Упаковка,
		|	ЕСТЬNULL(КурсыВалютСрезПоследних.Курс, 1) КАК Курс,
		|	ЕСТЬNULL(КурсыВалютСрезПоследних.Кратность, 1) КАК Кратность
		|ИЗ
		|	втЦеныНоменклатуры КАК ЦеныНоменклатуры
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Период, Валюта = ЗНАЧЕНИЕ(Справочник.Валюты.ВалютаРегламентированногоУчета)) КАК КурсыВалютСрезПоследних
		|		ПО ИСТИНА
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СтоимостьПоСкладу КАК ВТ_СтоимостьПоСкладу
		|		ПО ЦеныНоменклатуры.Номенклатура = ВТ_СтоимостьПоСкладу.Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СтоимостьПоПодразделению КАК ВТ_СтоимостьПоПодразделению
		|		ПО ЦеныНоменклатуры.Номенклатура = ВТ_СтоимостьПоПодразделению.Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СтоимостьПоОрганизации КАК ВТ_СтоимостьПоОрганизации
		|		ПО ЦеныНоменклатуры.Номенклатура = ВТ_СтоимостьПоОрганизации.Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЗакупкиТекМесяцСклад КАК ВТ_ЗакупкиТекМесяцСклад
		|		ПО ЦеныНоменклатуры.Номенклатура = ВТ_ЗакупкиТекМесяцСклад.Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЗакупкиТекМесяцПодразделение КАК ВТ_ЗакупкиТекМесяцПодразделение
		|		ПО ЦеныНоменклатуры.Номенклатура = ВТ_ЗакупкиТекМесяцПодразделение.Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЗакупкиТекМесяцОрганизация КАК ВТ_ЗакупкиТекМесяцОрганизация
		|		ПО ЦеныНоменклатуры.Номенклатура = ВТ_ЗакупкиТекМесяцОрганизация.Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЗакупкиПредМесяцСклад КАК ВТ_ЗакупкиПредМесяцСклад
		|		ПО ЦеныНоменклатуры.Номенклатура = ВТ_ЗакупкиПредМесяцСклад.Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЗакупкиПредМесяцПодразделение КАК ВТ_ЗакупкиПредМесяцПодразделение
		|		ПО ЦеныНоменклатуры.Номенклатура = ВТ_ЗакупкиПредМесяцПодразделение.Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЗакупкиПредМесяцОрганизация КАК ВТ_ЗакупкиПредМесяцОрганизация
		|		ПО ЦеныНоменклатуры.Номенклатура = ВТ_ЗакупкиПредМесяцОрганизация.Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СтоимостьТолькоНоменклатура КАК ВТ_СтоимостьТолькоНоменклатура
		|		ПО ЦеныНоменклатуры.Номенклатура = ВТ_СтоимостьТолькоНоменклатура.Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЗакупкиТекМесяцТолькоНоменклатура КАК ВТ_ЗакупкиТекМесяцТолькоНоменклатура
		|		ПО ЦеныНоменклатуры.Номенклатура = ВТ_ЗакупкиТекМесяцТолькоНоменклатура.Номенклатура
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЗакупкиПредМесяцТолькоНоменклатура КАК ВТ_ЗакупкиПредМесяцТолькоНоменклатура
		|		ПО ЦеныНоменклатуры.Номенклатура = ВТ_ЗакупкиПредМесяцТолькоНоменклатура.Номенклатура";
		
		// Заменяем основной запрос на запрос себестоимости
		Запрос.Текст = СтрШаблон(ТекстЗапросаШаблон, ТекстЗапросаЦены, ТекстЗапросаСебестоимости);
		
	Иначе
		
		// Выполняем стандартный запрос по ценам номенклатуры
