...
Функция ПолучитьСтоимостьПеределаПоРецептуре(Рецептура, 
	...
	ГлубинаЦикла = 0, 
	РассчетПоСебестоимости = Ложь) Экспорт
	...
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		...
		Если ЗначениеЗаполнено(СтрокаДерева.Рецептура) Тогда
			...
			Цена = ПолучитьСтоимостьПеределаПоРецептуре(СтрокаДерева.Рецептура, ВидЦены, Дата, Валюта, КэшЦен,, НеИспользоватьСезонныеПроценты, КэшЗаполненияДерева, УточненияАвтовыбора, СкладДляРасчетаЦены, РазворачиватьДоИнгредиентов, ГлубинаЦикла + 1, РассчетПоСебестоимости);
			// >> MRS #Ошибка данных ДенисламовАД <Adel.Denislamov@mriyaresort.com> 09.06.25, ()
			Цена = Цена / (СтрокаДерева.Рецептура.Количество);
		КонецЕсли;
		Если Цена = 0  Тогда
			Цена = ПолучитьСтоимостьИнгредиента(СтрокаДерева.Номенклатура, ВидЦены, Дата, Валюта,,,,,Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка(),СтрокаДерева.Характеристика, РассчетПоСебестоимости);
...
Функция ПолучитьСтоимостьИнгредиента(Номенклатура, 
	...
	СтруктураПараметровЦены.Вставить("РассчетПоСебестоимости", РассчетПоСебестоимости); // Швецов. 22.01.26 ERP-5547. Расчет по себестоимости для цен меню
