...
Процедура КомандаПечатьПриказаЕЛМА(Форма) Экспорт
	...
	// Формируем печатную форму приказа на сервере
	Адрес = MRS_ИнтеграцияЕЛМАСервер.СформироватьПриказЕЛМА(Форма.Объект.Ссылка);
	
	Если ПустаяСтрока(Адрес) Тогда
		ПоказатьПредупреждение(, "Не удалось сформировать приказ. Проверьте наличие макета печати.");
		Возврат;
	КонецЕсли;
	
	// Открываем сформированный документ Word
	// Вариант 1: Через БСП (предпочтительно, если доступна подсистема РаботаСФайлами)
	Попытка
