...
Процедура ПослеВыбораПараметровПриказа(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если РезультатОтправки.Успех Тогда
		...
		ПоказатьОповещениеПользователя(
			...
			,
			СтрШаблон("Документ успешно отправлен в систему ЕЛМА. УИД процесса: %1", РезультатОтправки.УИДЕЛМА),
...
//
// Отправляет документ в ЕЛМА немедленно (серверный вызов)
//
// Параметры:
//  ДокументСсылка - ДокументСсылка.УстановкаЦенНоменклатуры - документ
//  ПараметрыПриказа - Структура - параметры приказа
//
// Возвращаемое значение:
//  Структура:
//   * Успех - Булево
...
Функция ПолучитьРанееСохраненныеПараметрыВызовСервера(ДокументСсылка)
	...
	Возврат MRS_ИнтеграцияЕЛМАСервер.ПолучитьСохраненныеПараметрыПриказа(ДокументСсылка);
