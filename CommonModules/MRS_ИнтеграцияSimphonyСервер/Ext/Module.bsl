...
Функция ОбновитьВидимостьПозиции(Позиция, ЗначениеВыключен) Экспорт
	...
	ЗапросУзлов.Текст = 
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланОбмена.питУдаленныеКассы.питМеню КАК питМенюУзлов
		|		ПО питУдаленныеКассы.Ссылка = питМенюУзлов.Ссылка
		|ГДЕ
		|	питМенюУзлов.питМеню = &ВидМеню
		|	И НЕ питУдаленныеКассы.ПометкаУдаления";
	...
	ЗапросСтатуса.УстановитьПараметр("КассовыйУзел", ВыборкаУзлов.КассовыйУзел);
	...
	// Получаем цену для данного узла
