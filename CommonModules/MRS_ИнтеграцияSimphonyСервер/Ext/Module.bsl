...
// Процедура регламентного задания для автоматической синхронизации марок ЕГАИС
// Вызывается каждые 5 минут для обработки очереди
//
Процедура СинхронизироватьМаркиЕГАИСИзОчереди() Экспорт
	...
	Для Каждого СтрокаДокумента Из ТаблицаОчереди Цикл
		...
		Попытка
			...
			Если Не ЗначениеЗаполнено(СтрокаДокумента.Документ) Тогда
				...
				УдалитьИзОчередиМарок(СтрокаДокумента.Документ, СтрокаДокумента.КассовыйУзел);
			...
			Если Не ЗначениеЗаполнено(Номенклатура) Тогда
				...
				ОбработатьОшибкуСинхронизацииМарок(СтрокаДокумента.Документ, СтрокаДокумента.КассовыйУзел, 0, ТекстОшибки);
				ВсегоОшибок = ВсегоОшибок + 1;
				Продолжить;
				
			КонецЕсли;
			
			ПозицияМеню = НайтиПозициюМенюПоНоменклатуре(Номенклатура, СтрокаДокумента.КассовыйУзел);
			...
			Если Не ЗначениеЗаполнено(ObjNum) Или ObjNum = 0 Тогда
				
				ТекстОшибки = "Не заполнен MRS_КодSimphony для позиции меню";
				ОбработатьОшибкуСинхронизацииМарок(СтрокаДокумента.Документ, СтрокаДокумента.КассовыйУзел, 0, ТекстОшибки);
				ВсегоОшибок = ВсегоОшибок + 1;
				Продолжить;
			...
			RvcId = Число(СокрЛП(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаДокумента.КассовыйУзел, "Префикс")));
			
			РезультатСинхронизации = СинхронизироватьМаркиЕГАИС(ПозицияМеню, СтрокаДокумента.КассовыйУзел, ObjNum, RvcId);
			...
			Иначе
				...
				ОбработатьОшибкуСинхронизацииМарок(СтрокаДокумента.Документ, СтрокаДокумента.КассовыйУзел, 0, РезультатСинхронизации.Сообщение);
				ВсегоОшибок = ВсегоОшибок + 1;
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации("MRS.Simphony.EGAIS.Marks",
				...
				,
				СтрокаДокумента.Документ,
				СтрШаблон("Ошибка синхронизации марок: %1", ТекстОшибки));
...
// Обработчик события ПриЗаписи для документов движения марок
//
// Параметры:
//  Источник - ДокументОбъект - документ-источник события
//
Процедура ПриЗаписиДокументовДвиженияМарок(Объект, Отказ) Экспорт
	
	Возврат; // TODO. Временно, убрать
	...
	Если Объект.ДополнительныеСвойства.Свойство("РежимЗаписи", РежимЗаписи) Тогда
	...
	Если ТипДокумента = Тип("ДокументОбъект.ПеремещениеТоваров") Тогда
		...
		ОбработатьПеремещениеТоваров(Объект);
		
	ИначеЕсли ТипДокумента = Тип("ДокументОбъект.ВнутреннееПотребление") и Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеТоваровПоТребованию Тогда
		
		ОбработатьВнутреннееПотребление(Объект);
		
	КонецЕсли;
	
КонецПроцедуры

// Обрабатывает документ ПеремещениеТоваров и ставит в очередь синхронизации марок
//
// Параметры:
//  Документ - ДокументОбъект.ПеремещениеТоваров - документ для обработки
//
Процедура ОбработатьПеремещениеТоваров(Документ)
	
	КассовыйУзелПолучатель = НайтиКассовыйУзелПоСкладу(Документ.СкладПолучатель);
	КассовыйУзелОтправитель = НайтиКассовыйУзелПоСкладу(Документ.СкладОтправитель);
...
// Обрабатывает документ ВнутреннееПотребление и ставит в очередь синхронизации марок
//
// Параметры:
//  Документ - ДокументОбъект.ВнутреннееПотребление - документ для обработки
//
...
// Ставит документ в очередь синхронизации марок ЕГАИС
//
// Параметры:
//  Документ - ДокументСсылка - документ для постановки в очередь
//  КассовыйУзел - ПланОбменаСсылка.питУдаленныеКассы - кассовый узел
//  Склад - СправочникСсылка.Склады - склад
//  ВидДвижения - ПеречислениеСсылка.ВидыДвиженияНакопления - вид движения
//
...
// Находит кассовый узел плана обмена питУдаленныеКассы по складу реализации
//
// Параметры:
//  Склад - СправочникСсылка.Склады - склад для поиска
//
// Возвращаемое значение:
//  ПланОбменаСсылка.питУдаленныеКассы или Неопределено - найденный кассовый узел
//
...
// Определяет вид движения марок (Приход/Расход) на основе типа документа и склада
//
// Параметры:
//  Документ - ДокументСсылка - документ для определения вида движения
//  Склад - СправочникСсылка.Склады - склад для анализа
//
// Возвращаемое значение:
//  ПеречислениеСсылка.ВидыДвиженияНакопления - вид движения (Приход или Расход)
//
Функция ОпределитьВидДвиженияМарок(Документ, Склад) Экспорт
	...
	Если ТипДокумента = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
		...
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Документ, "СкладОтправитель, СкладПолучатель");
		
		Если Реквизиты.СкладОтправитель = Склад Тогда
...
// Удаляет запись из очереди синхронизации марок ЕГАИС
//
// Параметры:
//  Документ - ДокументСсылка - документ для удаления из очереди
//  КассовыйУзел - ПланОбменаСсылка.питУдаленныеКассы - кассовый узел
//
...
// Обрабатывает ошибку синхронизации марок с увеличением счетчика попыток
//
// Параметры:
//  Документ - ДокументСсылка - документ с ошибкой синхронизации
//  КассовыйУзел - ПланОбменаСсылка.питУдаленныеКассы - кассовый узел
//  HTTPСтатус - Число - HTTP статус ответа (0 для таймаутов)
...
Процедура ОбработатьОшибкуСинхронизацииМарок(Документ, КассовыйУзел, HTTPСтатус, ТекстОшибки) Экспорт
	...
	Иначе
		...
		Запись.Статус = Перечисления.MRS_СтатусыСинхронизацииSimphony.Ошибка;
		Запись.СледующаяПопытка = ТекущаяДатаСеанса() + 300;
...
// Проверяет, является ли номенклатура алкогольной продукцией с обязательной маркировкой
//
// Параметры:
...
// Определяет номенклатуру из документа для указанного склада
//
// Параметры:
//  Документ - ДокументСсылка - документ для анализа
//  Склад - СправочникСсылка.Склады - склад для отбора
//
// Возвращаемое значение:
//  СправочникСсылка.Номенклатура или Неопределено - найденная номенклатура
//
...
// Находит позицию меню по номенклатуре и кассовому узлу
//
// Параметры:
...
//
// Возвращаемое значение:
//  СправочникСсылка.питМеню или Неопределено - найденная позиция меню
//
...
Функция ЭтоАлкогольнаяПродукция(Позиция) Экспорт
	...
	Номенклатура = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Позиция, "Номенклатура");
	
	Возврат ЭтоАлкогольнаяПродукцияСМаркировкой(Номенклатура);
...
// Получает марки ЕГАИС номенклатуры
// Параметры:
//  Номенклатура - СправочникСсылка.Номенклатура - позиция номенклатуры
//  КассовыйУзел - ПланОбменаСсылка.питУдаленныеКассы - кассовый узел для получения склада реализации
//
// Возвращаемое значение:
//  Массив из Структура - массив марок ЕГАИС с остатками на складе:
//   * code - Строка - код акцизной марки ЕГАИС из Серия.Номер
Функция ПолучитьМаркиЕГАИСИз1С(Номенклатура, КассовыйУзел)
	...
	// Получить склад реализации из плана обмена касс ККМ
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТоварыНаСкладахОстатки.Серия.Номер КАК КодМарки
	|ИЗ
	...
	Запрос.УстановитьПараметр("Склад", СкладРеализации);
	Запрос.УстановитьПараметр("Номенклатура", Номенклатура);
	...
	Пока Выборка.Следующий() Цикл
		...
		// Пропускаем строки с пустым кодом марки
		Если Не ЗначениеЗаполнено(Выборка.КодМарки) Тогда
			Продолжить;
		КонецЕсли;
		
		// Формируем структуру с полями для синхронизации с Simphony
		СтруктураМарки = Новый Структура;
		СтруктураМарки.Вставить("code", Выборка.КодМарки);
		
		Результат.Добавить(СтруктураМарки);
...
Функция СинхронизироватьМаркиЕГАИС(Позиция, КассовыйУзел, ObjNum, RvcId)
	...
	Попытка
		...
		ДанныеПозиции = ПолучитьДанныеПозицииЕГАИС(Позиция);
		Марки1С = ПолучитьМаркиЕГАИСИз1С(ДанныеПозиции.Номенклатура, КассовыйУзел);
...
Функция СинхронизироватьШтрихкоды(Позиция, КассовыйУзел, ObjNum, RvcId)
	...
	Попытка
		...
		ДанныеПозиции = ПолучитьДанныеПозицииЕГАИС(Позиция);
		Штрихкоды1С = ПолучитьШтрихкодыИз1С(ДанныеПозиции.Номенклатура);
