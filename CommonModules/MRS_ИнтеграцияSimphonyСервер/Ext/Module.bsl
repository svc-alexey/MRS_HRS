...
Функция ВыгрузитьПозициюВSimphony(Позиция, КассовыйУзел) Экспорт
	...
	Попытка
		...
		Если ЗначениеЗаполнено(Позиция.MRS_КодSimphony) Тогда
			Операция = "UPDATE";
		КонецЕсли;
...
//
// Возвращаемое значение:
//  Число - hierStrucId (по умолчанию = 1)
//
Функция ПолучитьКодSimphonyУзла(КассовыйУзел)
	
	// RVC код берется из префикса питКассовыйУзел
	Возврат 1;
	
КонецФункции

// Получает RVC код кассового узла
//
// Параметры:
//  КассовыйУзел - ПланОбменаСсылка.питУдаленныеКассы
//
// Возвращаемое значение:
//  Строка - RVC код из реквизита Префикс
//
// Примечание:
//  Функция подготовлена для будущего использования.
//  В текущей версии API Simphony нет реквизита для конкретной точки продажи (RVC).
