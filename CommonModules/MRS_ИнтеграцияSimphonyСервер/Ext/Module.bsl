	...
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		...
		ИначеЕсли ТекВидНоменклатуры = Перечисления.питВидыНоменклатуры.Блюдо Тогда
			...
			Если ЗначениеЗаполнено(РецептураПередела) Тогда
				...
				СуммаНорма				= питПроизводство.ПолучитьСтоимостьПеределаПоРецептуре(РецептураПередела, ТипЦенНормативная, ДатаКалькуляции, Валюта,,,НеИспользоватьСезонныеПроценты, КэшЗаполненияДерева, УточненияАвтовыбора, Склад, РазворачиватьДоИнгредиентов, 19);
				Цена					= СуммаНорма / ?(КоличествоВРецептуре=0,1,КоличествоВРецептуре);
			Иначе
				Цена = питПроизводство.ПолучитьСтоимостьИнгредиента(СтрокаДерева.Номенклатура, ТипЦенНормативная, ДатаКалькуляции, Валюта , ,,,,ЕдиницаИзмерения,СтрокаДерева.Характеристика);	
			КонецЕсли; 	
		Иначе
			Цена = питПроизводство.ПолучитьСтоимостьИнгредиента(СтрокаДерева.Номенклатура, ТипЦенНормативная, ДатаКалькуляции, Валюта , ,,,,ЕдиницаИзмерения,СтрокаДерева.Характеристика);
		КонецЕсли;
