...
//   * HTTPСтатус - Число - код HTTP ответа
//
Функция УдалитьПозициюИзSimphonyСинхронно(StringNumberId, HierStrucId, Наименование = "") Экспорт
	...
	Попытка
		...
		Если HTTPОтвет.КодСостояния = 200 Тогда
			...
			ЗаписьЖурналаРегистрации("MRS.Simphony.Удаление", УровеньЖурналаРегистрации.Информация,,,
				СтрШаблон("Удалено: %1 (ID: %2, hierStrucId: %3)", Наименование, StringNumberId, HierStrucId));
		...
		ИначеЕсли HTTPОтвет.КодСостояния = 404 Тогда
			...
			ЗаписьЖурналаРегистрации("MRS.Simphony.Удаление", УровеньЖурналаРегистрации.Предупреждение,,,
				СтрШаблон("Позиция не найдена: %1 (ID: %2)", Наименование, StringNumberId));
		...
		ИначеЕсли HTTPОтвет.КодСостояния = 400 Тогда
			...
			ЗаписьЖурналаРегистрации("MRS.Simphony.Удаление", УровеньЖурналаРегистрации.Ошибка,,,
				СтрШаблон("Ошибка удаления: %1 - %2", Наименование, Результат.Сообщение));
		...
		ИначеЕсли HTTPОтвет.КодСостояния = 500 Тогда
			...
			ЗаписьЖурналаРегистрации("MRS.Simphony.Удаление", УровеньЖурналаРегистрации.Ошибка,,,
				СтрШаблон("Ошибка сервера: %1 - %2", Наименование, Результат.Сообщение));
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации("MRS.Simphony.Удаление", УровеньЖурналаРегистрации.Ошибка,,,
				СтрШаблон("Ошибка HTTP %1: %2 - %3", HTTPОтвет.КодСостояния, Наименование, Результат.Сообщение));
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации("MRS.Simphony.Удаление", УровеньЖурналаРегистрации.Ошибка,,,
			СтрШаблон("Ошибка соединения: %1 - %2", Наименование, ТекстОшибки));
...
//
// Параметры:
//  Объект - ДокументСсылка.УстановкаЦенНоменклатуры
Процедура ПоставитьВОчередьВыгрузкиНаСервереПриИзмененииЦен(Объект) Экспорт
	...
	КассовыйУзел = ПолучитьДанныеПоВидуЦен(ВидЦены);
	
	ТаблицаПозицийСЦенами = Объект.Товары;
	
	Попытка
	// Постановка позиций в очередь с проверкой цены
	Для Каждого СтрокаПозиции Из ТаблицаПозицийСЦенами Цикл
		
		Если СтрокаПозиции.Цена Тогда
			
			MRS_ИнтеграцияSimphonyСервер.ДобавитьВОчередьВыгрузки(СтрокаПозиции.Номенклатура, "CREATE", КассовыйУзел);
...
Функция ПолучитьДанныеПоВидуЦен(ВидЦены) Экспорт
	...
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
		КассовыйУзел = Выборка.КассовыйУзел;
	КонецЦикла;
	
	Возврат КассовыйУзел;
