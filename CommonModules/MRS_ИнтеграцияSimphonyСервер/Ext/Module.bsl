...
Функция УдалитьПозициюИзSimphonyСинхронно(StringNumberId, HierStrucId, Наименование = "") Экспорт
	...
	Попытка
		...
		Endpoint = СтрШаблон("/menu-item/%1?hierStrucId=%2", StringNumberId, HierStrucId);
...
// Удаляет позицию меню из Simphony (устаревшая, для совместимости с очередью)
//
// Параметры:
...
Функция УдалитьПозициюИзSimphony(Позиция, КассовыйУзел) Экспорт
	...
	Попытка
		...
		// Формирование endpoint с параметрами
		Endpoint = СтрШаблон("/menu-item/%1?hierStrucId=%2", КодSimphony, HierStrucId);
