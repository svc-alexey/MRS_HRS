...
Функция ПолучитьСтоимостьПоРецептуре(СтрокаДанных) Экспорт
	...
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		...
		НаКоличествоБлюдаНетто = Окр(СтрокаДерева.КоличествоУпаковокНетто * КоэффициентПересчета*СезонныйКоэффициент*КоэффициентРазделки, 3);
		...
		Сумма	= Цена * НаКоличествоБлюда;
		ИтогоСумма = ИтогоСумма + Окр(Цена * НаКоличествоБлюда, 2);

			
	КонецЦикла;
	
	// Швецов. 22.01.26 ERP-5547. Применение НДС и наценки к себестоимости
	Если РасчетСебестоимостиСтруктура <> Неопределено Тогда
		
		// Получаем ставку НДС из справочника питМеню по номенклатуре блюда
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	питМеню.MRS_СтавкаНДС.Ставка КАК СтавкаНДС
		|ИЗ
		|	Справочник.питМеню КАК питМеню
		|ГДЕ
		...
		// Получаем процент наценки из настроек удаленной кассы
		ПроцентНаценки = 0;
		
		Если СтрокаДанных.Свойство("ПроцентНаценки") Тогда
			
			ПроцентНаценки = СтрокаДанных.ПроцентНаценки;
			
		ИначеЕсли СтрокаДанных.Свойство("ВидЦен") Тогда
			
			// Получаем процент наценки по виду цен из настроек удаленной кассы
			ЗапросНаценки = Новый Запрос;
			ЗапросНаценки.Текст = 
			"ВЫБРАТЬ ПЕРВЫЕ 1
			|	питУдаленныеКассыMRS_КостПродаж.ПроцентНаценки КАК ПроцентНаценки
			|ИЗ
			|	ПланОбмена.питУдаленныеКассы.MRS_КостПродаж КАК питУдаленныеКассыMRS_КостПродаж
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланОбмена.питУдаленныеКассы КАК питУдаленныеКассы
			|		ПО питУдаленныеКассыMRS_КостПродаж.Ссылка = питУдаленныеКассы.Ссылка
			|ГДЕ
			|	питУдаленныеКассы.ВидЦен = &ВидЦен";
			
			ЗапросНаценки.УстановитьПараметр("ВидЦен", СтрокаДанных.ВидЦен);
			
			РезультатЗапросаНаценки = ЗапросНаценки.Выполнить();
			
			Если НЕ РезультатЗапросаНаценки.Пустой() Тогда
				
				ВыборкаНаценки = РезультатЗапросаНаценки.Выбрать();
				
				Если ВыборкаНаценки.Следующий() Тогда
					ПроцентНаценки = ВыборкаНаценки.ПроцентНаценки;
				КонецЕсли;
