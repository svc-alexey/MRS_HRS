...
Процедура ОбработкаПроведенияСборкаТоваров(ЭтотОбъект, Отказ) Экспорт 
	...
	Если ЕстьСтатус14 Тогда
		...
		Для Каждого Строка Из ТаблицаМаркируемыхТоваров Цикл
			Если Строка.СтатусУказанияСерий = 14 Тогда
				...
				// Проверка на дробное количество упаковок (недопустимо для маркируемой продукции)
		...
		Попытка
			...
			КонецЕсли;
		Исключение
			// Реквизит MRS_АкцизнаяМарка может отсутствовать в конфигурации - игнорируем ошибку
			ИгнорироватьОшибку = Ложь; //BSLLS:UseLessVariables-off
		КонецПопытки;
	...
	ИначеЕсли ЭтотОбъект.СтатусУказанияСерий = 0 И ЕстьМаркируемыеВТоварах Тогда
		...
		// При вскрытии марки хранятся в ТЧ Серии (а не в MRS_ДополнительныеАкцизныеМарки)
