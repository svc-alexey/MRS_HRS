...
//
Процедура СозданиеВГОПоЗаказамНаПеремещение(Форма, Объект, ПараметрыЗаписи) Экспорт
	...
	ДокОбъектСФ = Документы.СчетФактураВыданный.ПустаяСсылка();
	...
	СтрокаТЧ.Вставить("ПодразделениеОтправитель", СтруктураЗаполнения.ПодразделениеОтправитель);
	СтрокаТЧ.Вставить("ПодразделениеПолучатель", СтруктураЗаполнения.ПодразделениеПолучатель);
	...
	// Синхронизация состояния с документом перемещения
	ДокОбъектРТУ.ПометкаУдаления = Объект.ПометкаУдаления;
	
	// Определение режима записи на основе состояния перемещения
	РежимЗаписиРТУ = ПараметрыЗаписи.РежимЗаписи;
	
	// Если документ перемещения проводится или отменяется, синхронизируем РТУ
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		РежимЗаписиРТУ = РежимЗаписиДокумента.Проведение;
	ИначеЕсли ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда
		РежимЗаписиРТУ = РежимЗаписиДокумента.ОтменаПроведения;
	Иначе
		РежимЗаписиРТУ = РежимЗаписиДокумента.Запись;
	КонецЕсли;
	
	ДокОбъектРТУ.Записать(РежимЗаписиРТУ);
	...
	Попытка
		...
		ИнициализироватьДокументПТУ(СтрокаТЧ, СтруктураЗаполнения, ДокОбъектПТУ, ДокОбъектРТУ, ДокОбъектСФ, ДатаПТУ);
	...
	// Синхронизация состояния с документом перемещения
	ДокОбъектПТУ.ПометкаУдаления = Объект.ПометкаУдаления;
	
	// Определение режима записи на основе состояния перемещения
	РежимЗаписиПТУ = ПараметрыЗаписи.РежимЗаписи;
	
	// Если документ перемещения проводится или отменяется, синхронизируем ПТУ
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		РежимЗаписиПТУ = РежимЗаписиДокумента.Проведение;
	ИначеЕсли ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда
		РежимЗаписиПТУ = РежимЗаписиДокумента.ОтменаПроведения;
	Иначе
		РежимЗаписиПТУ = РежимЗаписиДокумента.Запись;
	КонецЕсли;
	
	ДокОбъектПТУ.Записать(РежимЗаписиПТУ);
...
Функция ПроверитьСуществованиеЗаказаНаПеремещение(МассивСсылок) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДокументПТУ.ЗаказПоставщику КАК ЗаказПоставщику
	|ПОМЕСТИТЬ ВТ_Заказ
	|ИЗ
	|	Документ.ПриобретениеТоваровУслуг КАК ДокументПТУ
	|ГДЕ
	|	ДокументПТУ.Ссылка В(&Приобретение)
	|	И ДокументПТУ.ПоступлениеПоЗаказам
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Заказ.ДокументОснование КАК ЗаказНаПеремещение
	|ИЗ
	|	ВТ_Заказ КАК ВТ_Заказ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПоставщику КАК Заказ
	|		ПО ВТ_Заказ.ЗаказПоставщику = Заказ.Ссылка
	|ГДЕ
	|	Заказ.ДокументОснование ССЫЛКА Документ.ЗаказНаПеремещение
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЗаказТовары.ПЛ_Заказ
	|ИЗ
	|	ВТ_Заказ КАК ВТ_Заказ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПоставщику.Товары КАК ЗаказТовары
	|		ПО ВТ_Заказ.ЗаказПоставщику = ЗаказТовары.Ссылка
	|ГДЕ
	|	ЗаказТовары.ПЛ_Заказ ССЫЛКА Документ.ЗаказНаПеремещение";

	Запрос.УстановитьПараметр("Приобретение", МассивСсылок);
	...
	Иначе
		...
		Пока Выборка.Следующий() Цикл	
			МассивСсылок.Добавить(Выборка.ЗаказНаПеремещение);		
		КонецЦикла;
...
Процедура ЗаполнитьТабличнуюЧастьТовары(ДокументОбъект, ТаблицаТоваров)
	...
	Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		Если ТипДокумента = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			НоваяСтрока.СтавкаНДС = УчетНДСУПВызовСервера.СтавкаНДСПоУмолчанию(ДокументОбъект.Организация, ТекущаяДатаСеанса());
...
Процедура ИнициализироватьДокументРТУ(ДокОбъектРТУ, СтруктураЗаполнения, СтрокаТЧ, ДатаДокументов)
	...
	ДокОбъектРТУ.Статус = Перечисления.СтатусыРеализацийТоваровУслуг.Отгружено;
	ДокОбъектРТУ.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоНакладным;
	ДокОбъектРТУ.ВариантОформленияПродажи = Перечисления.ВариантыОформленияДокументовПродажи.РеализацияТоваровУслуг;
...
Функция ПолучитьТаблицуТоваровИзПермещения(Ссылка)
	...
	Текст = "ВЫБРАТЬ
	        ...
	        |	ВЫБОР
	        |		КОГДА ПеремещениеТоваров.СкладОтправитель.ПЛ_Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	        |			ТОГДА ПеремещениеТоваров.ДокументОснование.Организация
	        |		ИНАЧЕ ПеремещениеТоваров.СкладОтправитель.ПЛ_Организация
	        |	КОНЕЦ КАК ОрганизацияОтправитель,
	        |	ВЫБОР
	        |		КОГДА ПеремещениеТоваров.СкладПолучатель.ПЛ_Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	        |			ТОГДА ПеремещениеТоваров.ЗаказНаПеремещение.Организация
	        |		ИНАЧЕ ПеремещениеТоваров.СкладПолучатель.ПЛ_Организация
	...
	Пока ВыборкаШапка.Следующий() Цикл
		...
		ОрганизацияПолучатель = ВыборкаШапка.ОрганизацияПолучатель;
		ОрганизацияОтправитель = ВыборкаШапка.ОрганизацияОтправитель;
		...
		СтруктураВозврата.Вставить("ПодразделениеОтправитель", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВыборкаШапка.СкладОтправитель, "Подразделение"));
		СтруктураВозврата.Вставить("ПодразделениеПолучатель", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВыборкаШапка.СкладПолучатель, "Подразделение"));	
