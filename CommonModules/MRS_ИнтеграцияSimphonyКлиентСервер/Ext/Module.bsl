...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	Если Форма.ИмяФормы = "Документ.УстановкаЦенНоменклатуры.Форма.ФормаДокумента" Тогда
		...
		МассивДобавляемыхРеквизитов.Добавить(
			Новый РеквизитФормы("URLProcess", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(500))));
		...
		КнопкаURLProcess = Форма.Элементы.Добавить("КнопкаURLProcess", Тип("КнопкаФормы"), Форма.Элементы.ГруппаПодвал);	
		КнопкаURLProcess.Вид = ВидКнопкиФормы.Гиперссылка;
		КнопкаURLProcess.Заголовок = "Процесс ЕЛМА";
		
		КомандаURLProcess = Форма.Команды.Добавить("Подключаемый_ОткрытьURLProcess");
		КомандаURLProcess.Действие = "Подключаемый_ОткрытьURLProcess";
		КнопкаURLProcess.ИмяКоманды = "Подключаемый_ОткрытьURLProcess";
		...
		ДобавитьЭлементыФормы(Форма, "КомандаПечатьПриказаЕЛМА", "Подключаемый_КомандаПечатьПриказаЕЛМА", 
...
Процедура ЗагрузитьСтатусДокумента(Форма) Экспорт
	...
	#Если Сервер Тогда
		...
		Если СтруктураСтатуса <> Неопределено Тогда
			...
			Форма.URLProcess = СтруктураСтатуса.URLProcess;
			Форма.КомментарийЕЛМА = СтруктураСтатуса.КомментарийЕЛМА;
			
			Если ЗначениеЗаполнено(СтруктураСтатуса.URLProcess) Тогда
				Форма.Элементы.КнопкаURLProcess.Заголовок = СтруктураСтатуса.URLProcess;
				Форма.Элементы.КнопкаURLProcess.Видимость = Истина;
			Иначе
				Форма.Элементы.КнопкаURLProcess.Видимость = Ложь;
			КонецЕсли;
			
			Если СтруктураСтатуса.Статус = Перечисления.MRS_СтатусыСинхронизацииELMA.Подписан 
			ИЛИ СтруктураСтатуса.Статус = Перечисления.MRS_СтатусыСинхронизацииELMA.Отклонен 
			ИЛИ СтруктураСтатуса.Статус = Перечисления.MRS_СтатусыСинхронизацииELMA.Отменен Тогда 
...
Функция ПолучитьТекстСтатусаЕЛМА(СтатусЕЛМА) Экспорт
	...
	#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
		...
		ИначеЕсли СтатусЕЛМА = Перечисления.MRS_СтатусыСинхронизацииELMA.Отправлен Тогда
			Возврат "Отправлен (ожидание callback)";
...
Функция ВалидироватьДокументПередОтправкой(Документ, ПараметрыПриказа = Неопределено) Экспорт
	...
	// Массив обязательных полей для проверки
	МассивПолей = Новый Массив;
	МассивПолей.Добавить(Новый Структура("Имя, Описание", "ДатаНачалаДействия", "Не заполнена дата начала действия цен"));
	МассивПолей.Добавить(Новый Структура("Имя, Описание", "Тема", "Не заполнена Тема"));
	МассивПолей.Добавить(Новый Структура("Имя, Описание", "Цель", "Не заполнена Цель"));
	МассивПолей.Добавить(Новый Структура("Имя, Описание", "Организация", "Не заполнена Организация"));
	МассивПолей.Добавить(Новый Структура("Имя, Описание", "Склад", "Не заполнен Склад"));
	
	// Проверка заполненности обязательных полей
	Для Каждого ПолеДляПроверки Из МассивПолей Цикл
		
		Если НЕ ПараметрыПриказа.Свойство(ПолеДляПроверки.Имя) 
			ИЛИ НЕ ЗначениеЗаполнено(ПараметрыПриказа[ПолеДляПроверки.Имя]) Тогда
			
			Результат.МассивОшибок.Добавить(ПолеДляПроверки.Описание);
			
		КонецЕсли;
		
	КонецЦикла;
	
	// Специфическая проверка: дата не может быть в прошлом
...
Функция ТребуетсяАвтообновлениеСтатуса(СтатусЕЛМА) Экспорт
	...
	#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
		...
		// автообновление не требуется для статусов после "Отправлен"
		// так как ЕЛМА сама вызывает callback
		Если СтатусЕЛМА = Перечисления.MRS_СтатусыСинхронизацииELMA.ВОчереди
			Или СтатусЕЛМА = Перечисления.MRS_СтатусыСинхронизацииELMA.Отправлен Тогда
