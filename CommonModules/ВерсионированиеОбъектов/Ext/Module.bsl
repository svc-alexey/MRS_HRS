...
Функция ВосстановитьОбъектПоXML(ДанныеОбъекта, ТекстСообщенияОбОшибке = "")
	...
	Исключение
		...
		ТекстСообщенияОбОшибке = НСтр("ru = 'Не удалось перейти на выбранную версию.
											...
											|en = 'Не удалось перейти на выбранную версию.
											|Возможная причина: версия объекта была записана в другой версии приложения.
											|Техническая информация об ошибке: %1'");
...
Функция СведенияОВерсииОбъекта(Знач Ссылка, Знач НомерВерсии) Экспорт
	...
	Если Результат.ВерсияОбъекта = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Выбранная версия объекта отсутствует в приложении.';
								|en = 'Выбранная версия объекта отсутствует в приложении.'");
	КонецЕсли;
...
Функция ГраницыУдаленияОбъектов()
	...
	//>>Швецов. 30.05.2025. ERP-5058. Версионирование документов общепит
	//мы уже получили настройки версионирования запросом
	...
	//>>Швецов. 30.05.2025. ERP-5058. Версионирование документов общепит
	//перезатирает результат запроса
...
Функция ВывестиРеквизитыПоРазобранномуОбъекту(Результат, ВерсияОбъекта, СсылкаНаОбъект)
	...
	ВывестиТекстВОтчет(Результат, Секция, "R1C2", НСтр("ru = 'Реквизиты';
														|en = 'Реквизиты'"), ШрифтыСтиля.КрупныйШрифтТекста);
...
Функция ПерейтиНаВерсиюСервер(Ссылка, НомерВерсии, ТекстСообщенияОбОшибке, ОтменаПроведения = Ложь) Экспорт
	...
	// << MRS #HELP1C-8365 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 09.01.25, ()
	//Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
	Если Не Пользователи.ЭтоПолноправныйПользователь() И НЕ УправлениеДоступом.ЕстьРоль("MRS_ПереходНаВерсиюРазрешен") Тогда
	// >> MRS #HELP1C-8365 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 09.01.25, ()
	...
	Попытка
		...
		//>>Швецов. 23.04.2025. Откат версии для админа
		Если MRS_Сервер.РольДоступнаВызовСервера("АдминистраторСистемы") Тогда
			Объект.ДополнительныеСвойства.Вставить("ПроверкаДокументов_Отключить", Истина); 
			Объект.ДополнительныеСвойства.Вставить("ОтключитьМеханизмРегистрацииОбъектов", Истина);
			Объект.ДополнительныеСвойства.Вставить("питРазрешитьЗаписьТиповогоДокумента",Истина);
		КонецЕсли;
        //<<Швецов
