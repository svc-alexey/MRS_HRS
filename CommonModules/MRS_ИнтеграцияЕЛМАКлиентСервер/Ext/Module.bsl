...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	Если Форма.ИмяФормы = "Документ.УстановкаЦенНоменклатуры.Форма.ФормаДокумента" Тогда
		...
		ДобавитьЭлементыФормы(Форма, "КомандаОтправитьВЕЛМА", "Подключаемый_КомандаОтправитьВЕЛМА", "Отправить в ЕЛМА", "ФормаКоманднаяПанель", "ПодменюОрганайзер",,БиблиотекаКартинок.ВходящееСообщение); 
		ДобавитьЭлементыФормы(Форма, "КомандаОбновитьСтатусЕЛМА", "Подключаемый_КомандаОбновитьСтатусЕЛМА", "Обновить статус ЕЛМА", "ФормаКоманднаяПанель", "ПодменюОрганайзер",, БиблиотекаКартинок.ИсходящееСообщение);
...
Функция ВалидироватьДокументПередОтправкой(Документ, ПараметрыПриказа = Неопределено) Экспорт
	...
	Если ПараметрыПриказа <> Неопределено Тогда
		...
		// Проверка наличия списка к ознакомлению (опционально, может быть пустым)
		Если ПараметрыПриказа.Свойство("СписокКОзнакомлению") 
			...
			// Проверяем, что все записи в списке заполнены корректно
			Для Каждого СтрокаСписка Из ПараметрыПриказа.СписокКОзнакомлению Цикл
				
				Если НЕ ЗначениеЗаполнено(СтрокаСписка.ФизическоеЛицо) Тогда
					Результат.МассивОшибок.Добавить("В списке к ознакомлению есть незаполненные строки");
					Прервать;
				КонецЕсли;
