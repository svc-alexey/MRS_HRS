...
Функция ВалидироватьДокументПередОтправкой(Документ, ПараметрыПриказа = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыПриказа.Тема) Тогда
		Результат.МассивОшибок.Добавить("Не заполнена Тема");
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПараметрыПриказа.Цель) Тогда
		Результат.МассивОшибок.Добавить("Не заполнена Цель");
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПараметрыПриказа.Организация) Тогда
		Результат.МассивОшибок.Добавить("Не заполнена Организация");
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПараметрыПриказа.Склад) Тогда
		Результат.МассивОшибок.Добавить("Не заполнен Склад");
	КонецЕсли;
