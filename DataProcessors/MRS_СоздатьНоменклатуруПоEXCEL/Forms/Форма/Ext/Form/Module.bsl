...
Процедура КодОбработки()
	...
	Для Каждого СтрокаГруппы Из ТЗДляГруппировки Цикл
		...
		ТекущееНаименование = СтрокаГруппы.НовоеНаименование;
		ТекущийОбъемДАЛ = СтрокаГруппы.ОбъемДАЛ;
		...
		ОбъемУжеЕсть = Ложь;
		НаименованиеНРег = НРег(ТекущееНаименование);
		...
		// 2. Если объем не найден, добавляем его
		Если НЕ ОбъемУжеЕсть И ТекущийОбъемДАЛ > 0 Тогда
			
			ОбъемВЛитрах = ТекущийОбъемДАЛ * 10;
			...
			// 3. Ищем тег [mrs...] или [_mrs...], чтобы вставить объем перед ним
			ПозицияТегаMRS = СтрНайти(НРег(ТекущееНаименование), "[mrs");
			ПозицияТега_MRS = СтрНайти(НРег(ТекущееНаименование), "[_mrs");
			
			ПозицияТега = 0;
			Если ПозицияТегаMRS > 0 И ПозицияТега_MRS > 0 Тогда
				ПозицияТега = Мин(ПозицияТегаMRS, ПозицияТега_MRS);
			ИначеЕсли ПозицияТегаMRS > 0 Тогда
				ПозицияТега = ПозицияТегаMRS;
			ИначеЕсли ПозицияТега_MRS > 0 Тогда
				ПозицияТега = ПозицияТега_MRS;
			КонецЕсли;
			
			Если ПозицияТега > 0 Тогда
				// Тег найден, вставляем объем перед ним
				ОсноваНаименования = СокрП(Лев(ТекущееНаименование, ПозицияТега - 1));
				ТегЧасть = Сред(ТекущееНаименование, ПозицияТега);
				ТекущееНаименование = ОсноваНаименования + НовыйОбъемСтрока + " " + ТегЧасть;
			Иначе
				// Тег не найден, добавляем объем в конец
				ТекущееНаименование = СокрП(ТекущееНаименование) + НовыйОбъемСтрока;
			КонецЕсли;
			
			// 4. Проверяем и корректируем длину
			МаксДлинаНаименования = 150;
			Если СтрДлина(ТекущееНаименование) > МаксДлинаНаименования Тогда
				ТекущееНаименование = Лев(ТекущееНаименование, МаксДлинаНаименования);
			КонецЕсли;
			
			СтрокаГруппы.НовоеНаименование = ТекущееНаименование;
