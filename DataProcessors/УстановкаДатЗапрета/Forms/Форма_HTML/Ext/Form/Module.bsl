...
// Якорь раздела для прокрутки после пересборки HTML (команда моста).
&НаСервере
Перем HTML_ЯкорьПрокруткиПослеКоманды;
...
Процедура ПриОткрытии(Отказ)
	...
	ЧекпоинтСброситьЭкранПриОткрытии();
...
&НаКлиенте
Процедура ЧекпоинтОтложенноОткрытьРаботу() Экспорт
...
Процедура ПослеВводаНомераЗадачиИзHTML(Знач ВведенныйТекст, Знач ДопПараметры = Неопределено) Экспорт
	...
	HTML_ЯкорьПрокруткиКлиент = "sec-dates";
	ОбновитьОтображениеHTML();
...
Процедура УстановитьДатуЗапретаПользователям(Команда)
	...
	HTML_ЯкорьПрокруткиКлиент = "sec-queue";
...
Процедура ОбновитьИнформацию(Команда)
	...
	HTML_ЯкорьПрокруткиКлиент = "sec-db";
...
Процедура УбратьФлажки(Команда)
	...
	HTML_ЯкорьПрокруткиКлиент = "sec-orgs-pick";
...
Функция Чекпоинт_ТолькоЦифрыИзСтроки(Знач Текст)
	
	Стр = СокрЛП(Строка(Текст));
	Рез = "";
...
Процедура УстановитьДатуЗапретаПользователямНаСервере()
	...
	Для Каждого Строка из ДатыЗапрета Цикл
		...
		ДатаДляЗаписи = Строка.ДатаОткрытия;
		коментДляЗаписи = Строка.Комментарий;
	КонецЦикла;
	...
	НоваяЗапись.ДатаЗапрета 	= ДатаДляЗаписи;
	...
	НоваяЗапись.Комментарий 	= СтрШаблон("Номер заявки %1, временное разрешение. будет отключено в %2", коментДляЗаписи , Формат(ДатаДляЗаписи, "ДФ='(ddd) dd.MM.yyyy ЧЧ:мм'"));
...
&НаСервере
Функция HTML_ИдЯкоряПрокруткиДляКоманды(Знач Команда, Знач Имя)
	
	Если Имя = "selectBase" Или Имя = "togglePickBase" Или Имя = "removePickBase"
		Или Имя = "applyBases" Или Имя = "clearBasePick" Или Имя = "activateBase" Тогда
		Возврат "sec-bases";
	ИначеЕсли Имя = "togglePickUser" Или Имя = "removePickUser" Или Имя = "applyUsers"
		Или Имя = "clearUserPick" Или Имя = "activateUser" Тогда
		Возврат "sec-users";
	ИначеЕсли Имя = "pickO" Или Имя = "pickC" Или Имя = "calNav" Тогда
		Возврат "sec-dates";
	ИначеЕсли Имя = "flipOrg" Или Имя = "checkAll" Или Имя = "uncheckAll"
		Или Имя = "toggleOrg" Или Имя = "setOrgSel" Тогда
		Возврат "sec-orgs-pick";
	ИначеЕсли Имя = "tab" Тогда
		...
		КонецЕсли;
		Если КодВкладки = "t2" Тогда
			Возврат "sec-db";
		ИначеЕсли КодВкладки = "t3" Тогда
			Возврат "sec-http";
		Иначе
			Возврат "";
		КонецЕсли;
	ИначеЕсли Имя = "setField" Тогда
		...
		Если ТипЗнч(Команда) = Тип("Структура") Тогда
			НП = Строка(Команда.n);
		Иначе
			НП = Строка(Команда.Получить("n"));
		КонецЕсли;
		Если НП = "login" Или НП = "password" Тогда
			Возврат "sec-http";
		ИначеЕсли НП = "user" Тогда
			Возврат "sec-users";
		ИначеЕсли НП = "org" Тогда
			Возврат "sec-orgs-pick";
		ИначеЕсли НП = "task" Или НП = "dOpen" Или НП = "dClose" Тогда
			Возврат "sec-dates";
		Иначе
			Возврат "";
		КонецЕсли;
...
Процедура ВыполнитьКомандуИнтерфейсаНаСервере(Знач Команда)
	...
	HTML_ЯкорьПрокруткиПослеКоманды = HTML_ИдЯкоряПрокруткиДляКоманды(Команда, Имя);
...
Функция СтильHTMLИнтерфейса()
	...
	Возврат ":root{--bg:#eef1f6;--fg:#1a1d24;--b:#c8ced9;--a:#2563eb;--a2:#1d4ed8;--ok:#15803d;--ok2:#166534;--bad:#b91c1c;--bad2:#991b1b;}"
		...
		+ "input.inp-task{flex:1;min-width:140px;}"
		+ "a.task-lnk{flex-shrink:0;color:var(--a);font-size:12px;font-weight:600;text-decoration:none;white-space:nowrap;}"
		+ "a.task-lnk:hover{text-decoration:underline;}"
...
// Прокрутка к разделу после полной пересборки HTML: в поле HTML 1С прокрутка часто не на window — поднимаем предков с overflow.
&НаСервере
Функция HTML_СанитизацияИдЯкоряДляСкрипта(Знач Сырой)
	
	С = СокрЛП(Строка(Сырой));
	...
	Для Инд = 1 По СтрДлина(С) Цикл
		Сим = Сред(С, Инд, 1);
		К = КодСимвола(Сим);
		Если (К >= 48 И К <= 57) Или (К >= 65 И К <= 90) Или (К >= 97 И К <= 122) Или Сим = "-" Или Сим = "_" Тогда
			Р = Р + Сим;
		КонецЕсли;
	КонецЦикла;
	Возврат Р;
...
&НаСервере
Функция HTML_СкриптВосстановленияПрокруткиПослеМоста(Знач ИдЯкоря)
	
	Ид = HTML_СанитизацияИдЯкоряДляСкрипта(ИдЯкоря);
	Возврат "<script type=""text/javascript"">"
		+ "(function(){"
		+ "var A='" + Ид + "';"
		+ "function toId(id){if(!id)return;var el=document.getElementById(id);if(!el)return;"
		+ "function go(){"
		+ "try{el.scrollIntoView({block:'start',behavior:'auto'});}catch(e){try{el.scrollIntoView(true);}catch(e2){}}"
		+ "var se=document.scrollingElement||document.documentElement||document.body,y=0;"
		+ "try{y=el.getBoundingClientRect().top+(window.pageYOffset||se.scrollTop||0)-6;}catch(e){y=0;}"
		+ "try{window.scrollTo(0,y);}catch(e){}"
		+ "try{if(se)se.scrollTop=y;}catch(e){}"
		...
		+ "var p=el,i=0;while(p&&i++<12){p=p.parentElement;if(!p)break;"
		+ "var st=window.getComputedStyle?window.getComputedStyle(p):null;"
		+ "var oy=st&&(/auto|scroll|overlay/.test((st.overflowY||'')+(st.overflow||'')));"
		+ "if(oy&&p.scrollHeight>p.clientHeight+2){try{var er=el.getBoundingClientRect(),pr=p.getBoundingClientRect();p.scrollTop+=er.top-pr.top-4;}catch(e){}}"
		+ "}"
		+ "}"
		+ "go();setTimeout(go,0);setTimeout(go,50);setTimeout(go,200);}"
		+ "function run(){if(A)toId(A);}"
		+ "run();"
		+ "if(window.addEventListener){if(document.readyState==='complete'){setTimeout(run,0);setTimeout(run,80);}else window.addEventListener('load',function(){run();setTimeout(run,80);});}"
		+ "})();"
		+ "</script>";
...
Функция СобратьПолныйHTMLДокументНаСервере()
	...
	ЯкорьДляСкрипта = "";
	Если НЕ ПустаяСтрока(HTML_ЯкорьПрокруткиКлиент) Тогда
		ЯкорьДляСкрипта = СокрЛП(HTML_ЯкорьПрокруткиКлиент);
		HTML_ЯкорьПрокруткиКлиент = "";
	КонецЕсли;
	Если ПустаяСтрока(ЯкорьДляСкрипта) Тогда
		Попытка
			ЯкорьДляСкрипта = СокрЛП(Строка(HTML_ЯкорьПрокруткиПослеКоманды));
		Исключение
			ЯкорьДляСкрипта = "";
		КонецПопытки;
	КонецЕсли;
	HTML_ЯкорьПрокруткиПослеКоманды = "";
	
	HTML = HTML + HTML_СкриптВосстановленияПрокруткиПослеМоста(ЯкорьДляСкрипта) + "</body></html>";
	Возврат HTML;
...
Функция СформироватьHTMLВкладкаОсновнаяНаСервере(Знач БазаПодключена)
	...
	Ч = Ч + "<h2 id=""sec-bases"">База данных</h2>";
	...
	Ч = Ч + "<h2 id=""sec-orgs-ref"">Организации в базе (справка)</h2>";
	...
	Ч = Ч + "<h2 id=""sec-users"">Пользователь</h2>";
	...
	Ч = Ч + "<h2 id=""sec-dates"">Параметры периода и задачи</h2>";
	...
	Ч = Ч + "<p class=""hint"">Текущие даты и номер задачи (только просмотр в полях; даты — календарём выше, задачу — ссылкой «Изменить…»).</p>";
	СсылкаЗадача = HrefКомандыМоста(Новый Структура("c", "taskDlg"));
	Ч = Ч + "<div class=""date-inp-grid"">";
	Ч = Ч + "<span class=""lbl"">Открытие</span><input class=""inp-date"" type=""text"" readonly=""readonly"" tabindex=""-1"" value=""" + HTMLЭкранировать(ДатаВСтрокуHTML(параметр_ДатаОткрытия)) + """/>";
	Ч = Ч + "<span class=""lbl"">Отключение</span><input class=""inp-date"" type=""text"" readonly=""readonly"" tabindex=""-1"" value=""" + HTMLЭкранировать(ДатаВСтрокуHTML(параметр_ДатаОтключения)) + """/>";
	Ч = Ч + "<span class=""lbl"">Номер задачи</span>";
	Если ПустаяСтрока(параметр_НомерЗадачи) Тогда
		Ч = Ч + "<div class=""task-inp-cell""><input class=""inp-date inp-task"" type=""text"" readonly=""readonly"" tabindex=""-1"" placeholder=""не задан"" value=""""/><a class=""task-lnk"" href=""" + СсылкаЗадача + """>Изменить…</a></div>";
	Иначе
		Ч = Ч + "<div class=""task-inp-cell""><input class=""inp-date inp-task"" type=""text"" readonly=""readonly"" tabindex=""-1"" value=""" + HTMLЭкранировать(параметр_НомерЗадачи) + """/><a class=""task-lnk"" href=""" + СсылкаЗадача + """>Изменить…</a></div>";
	КонецЕсли;
	Ч = Ч + "</div>";
	
	Ч = Ч + "<h2 id=""sec-orgs-pick"">Организации (участвуют в добавлении строк)</h2>";
	...
	Ч = Ч + "<h2 id=""sec-queue"">Очередь отправки</h2>";
...
Функция СформироватьHTMLВкладкаИзБазыНаСервере()
	...
	Ч = "<h2 id=""sec-db"">Даты запрета в информационной базе</h2>";
...
Функция СформироватьHTMLВкладкаПараметрыНаСервере()
	...
	КонецЦикла;
	Ч = Ч + "</tbody></table>";
	Ч = Ч + "<h2 id=""sec-http"">Авторизация HTTP к выбранной базе</h2><div class=""grid"">";
