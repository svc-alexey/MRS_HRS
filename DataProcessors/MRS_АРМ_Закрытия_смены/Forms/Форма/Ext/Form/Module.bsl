...
&НаСервере
Процедура ИнициализироватьОтчетВедомость()
	
	// Получение схемы компоновки данных из отчета
	СхемаКомпоновкиДанных = Отчеты.ВедомостьПоТоварамНаСкладах1.ПолучитьМакет("ОсновнаяСхемаКомпоновкиДанных");
	
	// Помещение схемы во временное хранилище
	АдресСхемыКомпоновкиДанных = ПоместитьВоВременноеХранилище(СхемаКомпоновкиДанных, УникальныйИдентификатор);
	
	// Инициализация компоновщика настроек
	КомпоновщикНастроек.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(АдресСхемыКомпоновкиДанных));
	
	// Загрузка настроек по умолчанию, если они есть
	Если СхемаКомпоновкиДанных.НастройкиПоУмолчанию <> Неопределено Тогда
...
&НаСервере
Процедура УстановитьПараметрыОтчета(МассивСкладов)
	...
	// Очистка существующих отборов по складу
	Индекс = Настройки.Отбор.Элементы.Количество() - 1;
	Пока Индекс >= 0 Цикл
		
		ЭлементОтбора = Настройки.Отбор.Элементы[Индекс];
		
		Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			
			Если Строка(ЭлементОтбора.ЛевоеЗначение) = "Склад" Тогда
				Настройки.Отбор.Элементы.Удалить(Индекс);
				Прервать;
			КонецЕсли;
