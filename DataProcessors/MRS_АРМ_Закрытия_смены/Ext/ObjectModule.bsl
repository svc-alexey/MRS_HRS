...
// Формирует анализ данных по отрицательными остатками.
// Результат помещается в реквизит АнализДанных.
//
// Параметры:
//  МассивСкладов - Массив из СправочникСсылка.Склады - склады для отбора
//  УникальныйИдентификаторФормы - УникальныйИдентификатор - идентификатор формы для временного хранилища
...
//  Строка - адрес данных расшифровки во временном хранилище
//
Процедура СформироватьАнализ(МассивСкладов, УникальныйИдентификаторФормы) Экспорт
	...
	|ИЗ
	|	РегистрНакопления.ТоварыНаСкладах.ОстаткиИОбороты(&НачалоПериода, &КонецПериода, Регистратор, , Склад В (&МассивСкладов)) КАК ТоварыНаСкладахОстаткиИОбороты
	|ГДЕ
	...
	Построитель.Вывести(РезультатАнализа, АнализДанных);
