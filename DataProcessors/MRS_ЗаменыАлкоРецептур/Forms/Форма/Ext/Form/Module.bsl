...
#Область МногопоточнаяОбработкаРецептур

// Возвращает текст запроса для выборки рецептур с алкогольными ингредиентами для многопоточной обработки
//
// Возвращаемое значение:
//  Строка - текст SQL-запроса
//
&НаСервере
Функция ПолучитьЗапросДляМногопоточнойОбработкиРецептур() Экспорт
...
// Обрабатывает одну рецептуру - создает новую с заменой алкогольных ингредиентов
//
// Параметры:
//  СтрокаТаблицы - СтрокаТаблицыЗначений - строка из результата запроса с полями Ссылка и Номер
//
&НаСервере
Процедура ОбработатьРецептуруДляМногопоточногоВыполнения(СтрокаТаблицы) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	// Получаем объект исходной рецептуры
	РецептураИсходная = СтрокаТаблицы.Ссылка.ПолучитьОбъект();
	
	НовДок = Неопределено;
	
	// Читаем дополнительный реквизит MRS_НоваяРецептура
	СвойствоДопРеквиз = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("MRS_НоваяРецептура");
	ЗначениеСвойства = СокрЛП(УправлениеСвойствами.ЗначениеСвойства(РецептураИсходная.Ссылка, СвойствоДопРеквиз));
	
	// Ищем существующую новую рецептуру по номеру
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	                |	питРецептура.Ссылка КАК Ссылка
	                |ИЗ
	...
	// Если новый документ не найден, создаем копию исходной рецептуры
	Если НовДок = Неопределено Тогда
		НовДок = РецептураИсходная.Скопировать();
		НовДок.Дата = ТекущаяДатаСеанса();
		НовДок.ДатаНачала = КонецДня(ТекущаяДатаСеанса()) + 1;
	...
	// Собираем все вскрытые алкогольные номенклатуры для предварительной загрузки соответствий
	МассивВскрытыхНоменклатур = Новый Массив;
	Для Каждого СтрокаИнгредиенты Из НовДок.Ингредиенты Цикл
		Если СтрокаИнгредиенты.Номенклатура.АлкогольнаяПродукция И СтрокаИнгредиенты.Номенклатура.АлкогольнаяПродукцияВоВскрытойТаре Тогда
			МассивВскрытыхНоменклатур.Добавить(СтрокаИнгредиенты.Номенклатура);
	...
	// Предварительная загрузка всех соответствий одним запросом
	СоответствияНоменклатуры = Новый Соответствие;
	Если МассивВскрытыхНоменклатур.Количество() > 0 Тогда
		ЗапросСоответствий = Новый Запрос;
		ЗапросСоответствий.Текст = 
	...
	// Цикл по ингредиентам с заменой номенклатуры
	ЕстьЗамененныеИнгредиенты = Ложь;
	ТекстОшибки = "";
	...
	Для Каждого СтрокаИнгредиенты Из НовДок.Ингредиенты Цикл
		Если СтрокаИнгредиенты.Номенклатура.АлкогольнаяПродукция И СтрокаИнгредиенты.Номенклатура.АлкогольнаяПродукцияВоВскрытойТаре Тогда
			...
			Если Соответствие <> Неопределено Тогда
				...
				СтрокаИнгредиенты.Номенклатура = Соответствие.НоваяНоменклатура;
				ЕстьЗамененныеИнгредиенты = Истина;
	...
	Если ЕстьЗамененныеИнгредиенты Тогда
		...
		Попытка
			...
			// Записываем дополнительный реквизит MRS_НоваяРецептура
			ТЗ = Новый ТаблицаЗначений;
			ТЗ.Колонки.Добавить("Свойство", Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ДополнительныеРеквизитыИСведения"));
			ТЗ.Колонки.Добавить("Значение");
			НовСтр = ТЗ.Добавить();
	...
	Иначе
		...
		ТекстОшибки = ТекстОшибки + Символы.ПС + "Не найдено ни одного замененного ингредиента для создания новой рецептуры: " + РецептураИсходная.Ссылка;
		ВызватьИсключение ТекстОшибки;
